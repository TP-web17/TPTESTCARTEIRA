...
async function activateAuthenticatedSession(nextSession, options = {}) {
  ...
  storageSetItem(STORAGE_KEYS.ui, JSON.stringify({ lastUser: currentUser.username }));
...
function drawFinanceSeriesCanvas(canvas, series, options = {}) {
  ...
  auraPrimary.addColorStop(0, TP_CHART_THEME.orbA);
  ...
  auraSecondary.addColorStop(0, TP_CHART_THEME.orbB);
  ...
  for (let i = 0; i <= 4; i += 1) {
    ...
    ctx.fillStyle = TP_CHART_THEME.axisSoft;
  ...
  function drawSmoothSeries(values, color, fillColor) {
    ...
    ctx.lineCap = 'round';
    ctx.lineJoin = 'round';
    ctx.shadowColor = TP_CHART_THEME.shadow;
    ctx.shadowBlur = 16;
    ...
    values.forEach((value, index) => {
      ...
      ctx.beginPath();
      ctx.arc(x, y, isLastPoint ? 5.4 : 3.1, 0, Math.PI * 2);
      ctx.fillStyle = color;
      ctx.fill();

      ctx.beginPath();
      ctx.arc(x, y, isLastPoint ? 2.4 : 1.6, 0, Math.PI * 2);
  ...
  function drawLegendChip(x, y, color, text) {
    ...
    drawRoundedRect(x, y, chipWidth, chipHeight, chipHeight / 2);
    ctx.fillStyle = TP_CHART_THEME.pillBg;
    ctx.strokeStyle = TP_CHART_THEME.pillBorder;
    ctx.lineWidth = 1;
    ctx.fill();
    ctx.stroke();
  ...
  function drawTopTag() {
    const latestMonth = labels[labels.length - 1] || 'Atual';
    const tagText = `Base ${latestMonth}`;
    ctx.font = `${isNarrow ? 9 : 10}px ${CHART_BODY_FONT}`;
    const tagWidth = Math.max(82, Math.ceil(ctx.measureText(tagText).width + 24));
    const tagX = width - tagWidth - 12;
    drawRoundedRect(tagX, 12, tagWidth, 24, 12);
    ...
    ctx.textAlign = 'left';
    ctx.fillText(tagText, tagX + 12, 28);
  }
    ...
    <section class="panel wallet-panel">
      <div class="panel-header">
        <div>
          ...
          <p class="panel-subtitle">Resumo consolidado com grafico claro e leitura direta do mes atual.</p>
        </div>
      </div>
