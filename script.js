...
const PAGE_TITLES = {
  ...
  activityCenter: 'Central operacional',
  ...
  profile: 'Meu perfil',
  searchHub: 'Busca global'
};

const NAV_GROUP_TITLES = {
  overview: 'Visao geral',
  operation: 'Operacao',
  workspace: 'Equipe e rotina',
  control: 'Controle'
};

const NAV_LINK_META = {
  dashboard: { group: 'overview', icon: 'PG', hint: 'Financeiro e panorama' },
  activityCenter: { group: 'overview', icon: 'CO', hint: 'Alertas e prioridade' },
  myTickets: { group: 'operation', icon: 'TK', hint: 'Solicitacoes abertas' },
  createTicket: { group: 'operation', icon: 'AB', hint: 'Nova abertura' },
  pending: { group: 'operation', icon: 'PD', hint: 'Fila aguardando' },
  allTickets: { group: 'control', icon: 'GT', hint: 'Visao completa' },
  users: { group: 'control', icon: 'US', hint: 'Acessos e permissoes' },
  logs: { group: 'control', icon: 'LG', hint: 'Historico do sistema' },
  tasks: { group: 'workspace', icon: 'TF', hint: 'Planejamento interno' },
  notes: { group: 'workspace', icon: 'NT', hint: 'Anotacoes rapidas' },
  announcements: { group: 'workspace', icon: 'CM', hint: 'Avisos internos' },
  knowledge: { group: 'workspace', icon: 'KB', hint: 'Base de apoio' },
  tools: { group: 'workspace', icon: 'TL', hint: 'Acoes rapidas' },
  stealthChat: { group: 'workspace', icon: 'SC', hint: 'Conversa privada' },
  staffChat: { group: 'workspace', icon: 'CI', hint: 'Chat da equipe' },
  intelCenter: { group: 'control', icon: 'IT', hint: 'Leitura estrategica' },
  walletControl: { group: 'control', icon: 'CR', hint: 'Controle financeiro' },
  espionage: { group: 'control', icon: 'ES', hint: 'Monitoramento' },
  superTools: { group: 'control', icon: 'SM', hint: 'Ferramentas master' },
  profile: { group: 'workspace', icon: 'PF', hint: 'Conta e ajustes' }
};

const MOBILE_QUICK_LINKS = [
  { id: 'dashboard', label: 'Painel' },
  { id: 'activityCenter', label: 'Central' },
  { id: 'myTickets', label: 'Tickets' },
...
let FINANCE_CHART_YEAR = new Date().getFullYear();
const FINANCE_CHART_TYPES = ['line', 'bar', 'pie'];
...
const API_AUTH_LOGIN_ENDPOINT = '/api/auth/login';
const API_AUTH_SESSION_ENDPOINT = '/api/auth/session';
const API_AUTH_LOGOUT_ENDPOINT = '/api/auth/logout';
const API_AUTH_CHANGE_PASSWORD_ENDPOINT = '/api/auth/change-password';
const API_BOOTSTRAP_STATE_ENDPOINT = '/api/bootstrap-state';
...
const TP_CHART_THEMES = {
  light: {
    paidColor: '#6d28d9',
    paidFill: 'rgba(109, 40, 217, 0.18)',
    debtColor: '#c026d3',
    debtFill: 'rgba(192, 38, 211, 0.16)',
    bgTop: 'rgba(255, 255, 255, 0.98)',
    bgBottom: 'rgba(244, 238, 255, 0.98)',
    grid: 'rgba(124, 58, 237, 0.12)',
    label: '#4c1d95',
    axis: '#7c3aed',
    axisSoft: '#8b5cf6',
    orbA: 'rgba(124, 58, 237, 0.16)',
    orbB: 'rgba(217, 70, 239, 0.12)',
    shadow: 'rgba(76, 29, 149, 0.18)',
    pillBg: 'rgba(255, 255, 255, 0.9)',
    pillBorder: 'rgba(124, 58, 237, 0.16)'
  },
  dark: {
    paidColor: '#c084fc',
    paidFill: 'rgba(192, 132, 252, 0.2)',
    debtColor: '#f472b6',
    debtFill: 'rgba(244, 114, 182, 0.18)',
    bgTop: 'rgba(23, 14, 39, 0.98)',
    bgBottom: 'rgba(11, 7, 22, 0.99)',
    grid: 'rgba(196, 181, 253, 0.14)',
    label: '#f3e8ff',
    axis: '#ddd6fe',
    axisSoft: '#c4b5fd',
    orbA: 'rgba(168, 85, 247, 0.22)',
    orbB: 'rgba(244, 114, 182, 0.16)',
    shadow: 'rgba(3, 1, 11, 0.44)',
    pillBg: 'rgba(37, 22, 61, 0.92)',
    pillBorder: 'rgba(196, 181, 253, 0.18)'
  }
};
const CHART_BODY_FONT = 'Manrope';
const CHART_HEADING_FONT = 'Space Grotesk';
...
const els = {
  ...
  headerSearchButton: document.getElementById('header-search-button'),
...
let currentThemeMode = UI_THEME_LIGHT;
...
function readUiState() {
  const parsed = safeParse(storageGetItem(STORAGE_KEYS.ui), {});
...
function writeUiState(patch = {}) {
  ...
  storageSetItem(STORAGE_KEYS.ui, JSON.stringify(next));
  return next;
}
...
function getActiveChartTheme() {
  return currentThemeMode === UI_THEME_DARK ? TP_CHART_THEMES.dark : TP_CHART_THEMES.light;
}

function getThemeMetaColor(mode = currentThemeMode) {
  return mode === UI_THEME_DARK ? '#140b24' : '#7c3aed';
}

function normalizeFinanceChartType(type) {
  return FINANCE_CHART_TYPES.includes(type) ? type : 'line';
}

function getFinanceChartType() {
  return normalizeFinanceChartType(readUiState().financeChartType);
}

function setFinanceChartType(type) {
  const nextType = normalizeFinanceChartType(type);
  writeUiState({ financeChartType: nextType });
  if (session) {
...
function getFinanceChartTypeLabel(type) {
  if (type === 'bar') return 'Barras';
  if (type === 'pie') return 'Pizza';
  return 'Linha';
}

function renderFinanceChartTypeOptions(selectedType = getFinanceChartType()) {
  return FINANCE_CHART_TYPES
    .map((type) => `<option value="${type}" ${normalizeFinanceChartType(selectedType) === type ? 'selected' : ''}>${escapeHtml(getFinanceChartTypeLabel(type))}</option>`)
    .join('');
}

function updateThemeToggleButtons() {
  const nextLabel = currentThemeMode === UI_THEME_DARK ? 'Modo claro' : 'Modo escuro';
  [els.loginThemeToggle, els.themeToggleButton].forEach((button) => {
    if (!button) return;
    button.textContent = nextLabel;
    button.setAttribute('aria-pressed', currentThemeMode === UI_THEME_DARK ? 'true' : 'false');
    button.dataset.themeTarget = currentThemeMode === UI_THEME_DARK ? UI_THEME_LIGHT : UI_THEME_DARK;
  });
}
...
function updateThemeMetaTag() {
  ...
  if (themeMeta) {
    themeMeta.setAttribute('content', getThemeMetaColor());
  }
}
...
function updateConnectionStatusDisplay() {
  ...
  let state = 'offline';
  let label = 'Sincronizacao limitada';
  if (session) {
    if (presenceSocketConnected && presenceSocketAuthenticated) {
      state = 'online';
      label = 'Tempo real online';
    } else if (presenceSocketConnected) {
      state = 'syncing';
      label = 'Sincronizando';
    }
  }
...
function applyThemeMode(mode, options = {}) {
  ...
  if (persist) {
    writeUiState({ theme: normalizedMode });
  }
  if (rerender && session) {
    requestRealtimeRender();
  }
...
function toggleThemeMode() {
  applyThemeMode(currentThemeMode === UI_THEME_DARK ? UI_THEME_LIGHT : UI_THEME_DARK);
}

function restoreThemePreference() {
  const uiState = readUiState();
  applyThemeMode(uiState.theme || UI_THEME_LIGHT, { persist: false, rerender: false });
}
...
function getActivityCenterBadgeCount() {
  if (!session) return 0;
  const scopedTickets = getScopedTickets();
  const urgentTickets = scopedTickets.filter((ticket) => ticket.priority === 'urgent' && ticket.status !== 'closed').length;
  const overdueTasks = getUserTasks().filter((task) => task.status !== 'done' && task.dueDate && task.dueDate < getTodayIsoDate()).length;
  const unreadAnnouncements = announcements.filter((item) => {
    if ((Number(item.deletedAt) || 0) > 0) return false;
    return item.audience === 'all' || isAdminRole(session.role);
  }).length;
  return urgentTickets + overdueTasks + Math.min(3, unreadAnnouncements);
}
...
function setHeaderSearchValue(value) {
  globalSearchQuery = String(value || '').trim();
  if (els.headerSearchInput) {
    els.headerSearchInput.value = globalSearchQuery;
  }
}

function openGlobalSearch(query = '') {
  const nextQuery = String(query || (els.headerSearchInput ? els.headerSearchInput.value : '')).trim();
  setHeaderSearchValue(nextQuery);
  if (!session) return;
  navigate('searchHub');
}
...
function collectGlobalSearchResults(query) {
  ...
  scopedTickets.forEach((ticket) => {
    const haystack = buildSearchText([
      ...
      ticket.priority,
      ticket.status,
      ...(Array.isArray(ticket.messages) ? ticket.messages.map((message) => `${message.sender} ${message.content}`) : [])
    ]);
    ...
    results.push({
      ...
      meta: `${PRIORITY_LABELS[ticket.priority] || ticket.priority} | ${STATUS_LABELS[ticket.status] || ticket.status}`,
      description: toShortText(ticket.description || 'Sem descricao adicional.', 160),
      sortAt: Number(ticket.updatedAt) || Number(ticket.createdAt) || 0
    });
  });
  ...
  users.forEach((user) => {
    ...
    results.push({
      type: 'usuario',
      page: session && session.role === 'superadmin' ? 'users' : 'profile',
      title: `Usuario - ${getUserDisplayName(user)}`,
      meta: `@${user.username} | ${roleLabel(user.role)} | ${user.status}`,
      description: `Divida ${formatCurrency(user.debt)} | Pago ${formatCurrency(user.totalPaid)} | Lucro ${formatCurrency(user.walletProfit)}`,
      sortAt: Math.max(Number(user.profileUpdatedAt) || 0, Number(user.lastLoginAt) || 0, Number(user.roleUpdatedAt) || 0)
    });
  });

  getUserTasks().forEach((task) => {
    const haystack = buildSearchText([
      task.title,
      task.description,
      task.owner,
    ...
    results.push({
      ...
      page: 'tasks',
      title: task.title,
      meta: `${task.status} | ${task.visibility === TASK_VISIBILITY_PUBLIC ? 'publica' : 'privada'}`,
      description: toShortText(task.description || 'Sem descricao adicional.', 160),
      sortAt: Number(task.updatedAt) || Number(task.createdAt) || 0
    });
  });

  announcements.forEach((announcement) => {
    if ((Number(announcement.deletedAt) || 0) > 0) return;
    const haystack = buildSearchText([announcement.title, announcement.content, announcement.author, announcement.audience]);
    if (!haystack.includes(normalizedQuery)) return;
    results.push({
      type: 'comunicado',
      page: 'announcements',
      title: announcement.title,
      meta: `Por ${announcement.author || 'sistema'}`,
      description: toShortText(announcement.content || '', 160),
      sortAt: Number(announcement.createdAt) || 0
    });
  });

  logs.slice(-220).forEach((entry) => {
    const haystack = buildSearchText([entry.user, entry.action]);
    if (!haystack.includes(normalizedQuery)) return;
    results.push({
      type: 'log',
      page: canViewLogs() ? 'logs' : 'dashboard',
      title: entry.action,
      meta: `${entry.user} | ${formatDateTime(entry.timestamp)}`,
      description: 'Registro operacional encontrado na trilha de auditoria.',
      sortAt: Number(entry.timestamp) || 0
    });
  });
...
function initData() {
  ...
  if (Array.isArray(storedUsers) && storedUsers.length > 0) {
    ...
    users = [
      normalizeUser({ username: 'esther', role: 'superadmin', status: 'active', debt: 0 }),
      normalizeUser({ username: 'belle', role: 'admin', status: 'active', debt: 0 }),
      normalizeUser({ username: 'felps', role: 'member', status: 'active', debt: 0 }),
      normalizeUser({ username: 'inteligencia tp', role: 'inteligencia', status: 'active', debt: 0 })
    ];
  }
...
function normalizeUser(user) {
  ...
  const fallbackPasswordUpdatedAt = Number(source.passwordUpdatedAt) > 0 ? 1 : 0;
...
function ensureCoreUsers() {
  ...
  const requiredUsers = [
    { username: 'esther', role: 'superadmin', lockedRole: true },
    { username: 'belle', role: 'admin', lockedRole: false },
    { username: 'felps', role: 'member', lockedRole: false },
    { username: 'yoon', role: 'member', lockedRole: false },
    { username: 'murilo', role: 'member', lockedRole: false },
    { username: 'matheus', role: 'member', lockedRole: false },
    { username: 'inteligencia tp', role: 'inteligencia', lockedRole: true }
  ];
...
async function apiRequest(endpoint, options = {}) {
  if (!canLoadInitialServerState()) {
    throw new Error('API indisponivel neste ambiente.');
  }
  ...
  if (!response.ok) {
    const error = new Error(payload && payload.message ? payload.message : `Falha na requisicao (${response.status}).`);
...
async function loadInitialServerState() {
  ...
  try {
    ...
    const payload = await apiRequest(`${API_BOOTSTRAP_STATE_ENDPOINT}?ts=${Date.now()}`, {
...
function resetInitialServerStateTracking() {
  initialServerStateLoaded = false;
  initialServerStateLoadAttempted = false;
  initialServerStatePromise = null;
  remoteStateInitialized = false;
}

async function activateAuthenticatedSession(nextSession, options = {}) {
  const baseSession = nextSession && typeof nextSession === 'object' ? nextSession : {};
  session = {
    username: String(baseSession.username || '').trim().toLowerCase(),
    role: normalizeRoleKey(baseSession.role) || 'member',
    loginAt: Number(baseSession.loginAt) || Date.now()
  };

  const loadedServerState = await ensureInitialServerStateLoaded({ forceRetry: true });
  const hasAnyLocalState = !isAppStateEffectivelyEmpty(collectAppStateForSync());
  if (!loadedServerState && !hasAnyLocalState) {
    throw new Error('Nao foi possivel carregar os dados do servidor.');
  }

  const currentUser = getCurrentUser();
  if (!currentUser) {
    throw new Error('Usuario autenticado, mas nao encontrado nos dados carregados.');
  }

  if (options.registerAccess) {
    registerUserAccess(currentUser);
    saveData();
  }

  session.username = currentUser.username;
  session.role = currentUser.role;
  session.loginAt = Number(currentUser.lastLoginAt) || Number(baseSession.loginAt) || Date.now();
  writeUiState({ lastUser: currentUser.username });
  ...
  renderSidebar();
  navigate(options.page || 'dashboard');
  startClock();
...
async function restoreServerSession() {
  if (sessionRestoreInFlight || session || !canLoadInitialServerState()) return false;
  sessionRestoreInFlight = true;
  try {
    const payload = await apiRequest(API_AUTH_SESSION_ENDPOINT);
    if (!payload || !payload.user) return false;
    await activateAuthenticatedSession({
      username: payload.user.username,
      role: payload.user.role,
      loginAt: Number(payload.createdAt) || Date.now()
    }, {
    ...
    if (!error || error.status !== 401) {
      console.warn('[auth-session] Falha ao restaurar sessao.', error);
    }
    session = null;
    return false;
  } finally {
...
function createUserOnServer(username, role, password = DEFAULT_PASSWORD) {
  return apiRequest(API_ADMIN_USERS_ENDPOINT, {
    method: 'POST',
    body: {
      username,
      role,
...
function resetUserPasswordOnServer(username, newPassword) {
  return apiRequest(`${API_ADMIN_USERS_ENDPOINT}/${encodeURIComponent(username)}/password`, {
    method: 'POST',
    body: { newPassword }
  });
}

function changeOwnPasswordOnServer(currentPassword, newPassword) {
  return apiRequest(API_AUTH_CHANGE_PASSWORD_ENDPOINT, {
    method: 'POST',
    body: {
      currentPassword,
      newPassword
...
function mergeLocalUsersIntoRemoteState(remoteUsersRaw, localUsersRaw) {
  ...
  localUsers.forEach((localUser) => {
    ...
    if (localFinanceTs > remoteFinanceTs) {
      ...
      remoteUser.financeHistory = Array.isArray(localUser.financeHistory) ? localUser.financeHistory.slice(-1200) : [];
...
function connectPresenceSocket() {
  ...
  socket.addEventListener('open', () => {
    ...
    updateConnectionStatusDisplay();
    wsDebugLog('ws.open');
    sendPresenceSocketMessage({ type: 'auth' });
...
function getFinanceChartMonthKeys(year = FINANCE_CHART_YEAR) {
  const now = new Date();
  let limitMonth = 12;
  if (now.getFullYear() === year) {
    limitMonth = Math.max(1, now.getMonth() + 1);
  }
  const keys = [];
...
function isTimestampInFinanceChartYear(timestamp, year = FINANCE_CHART_YEAR) {
  const date = new Date(Number(timestamp) || 0);
  return date.getFullYear() === year;
}

function getFinanceChartScopeLabel() {
  const monthKeys = getFinanceChartMonthKeys();
  if (monthKeys.length === 0) return `Ano ${FINANCE_CHART_YEAR}`;
  const first = getMonthLabelFromKey(monthKeys[0]);
  const last = getMonthLabelFromKey(monthKeys[monthKeys.length - 1]);
...
function buildFinanceSeries(user, monthsCount = 8, anchorMonthKey = getCurrentMonthKey()) {
  ...
  months.forEach((month) => {
    ...
    paidSeries.push(Number(month.payments.toFixed(2)));
    chargeSeries.push(Number(month.charges.toFixed(2)));
...
function drawRoundedRectPath(ctx, x, y, boxWidth, boxHeight, radius) {
  const safeRadius = Math.min(radius, boxWidth / 2, boxHeight / 2);
  ctx.beginPath();
  ctx.moveTo(x + safeRadius, y);
  ctx.lineTo(x + boxWidth - safeRadius, y);
  ctx.quadraticCurveTo(x + boxWidth, y, x + boxWidth, y + safeRadius);
  ctx.lineTo(x + boxWidth, y + boxHeight - safeRadius);
  ctx.quadraticCurveTo(x + boxWidth, y + boxHeight, x + boxWidth - safeRadius, y + boxHeight);
  ctx.lineTo(x + safeRadius, y + boxHeight);
  ctx.quadraticCurveTo(x, y + boxHeight, x, y + boxHeight - safeRadius);
...
function drawFinanceSeriesCanvas(canvas, series, options = {}) {
  ...
  const ctx = canvas.getContext('2d');
  if (!ctx) return;
  const chartTheme = getActiveChartTheme();
  const chartType = normalizeFinanceChartType(options.chartType || getFinanceChartType());
  ...
  bgGradient.addColorStop(0, chartTheme.bgTop);
  bgGradient.addColorStop(1, chartTheme.bgBottom);
  ctx.fillStyle = bgGradient;
  ctx.fillRect(0, 0, width, height);

  const auraPrimary = ctx.createRadialGradient(width * 0.18, height * 0.16, 12, width * 0.18, height * 0.16, width * 0.56);
  auraPrimary.addColorStop(0, chartTheme.orbA);
  auraPrimary.addColorStop(1, 'rgba(255, 255, 255, 0)');
  ctx.fillStyle = auraPrimary;
  ctx.fillRect(0, 0, width, height);

  const auraSecondary = ctx.createRadialGradient(width * 0.82, height * 0.12, 8, width * 0.82, height * 0.12, width * 0.42);
  auraSecondary.addColorStop(0, chartTheme.orbB);
  auraSecondary.addColorStop(1, 'rgba(255, 255, 255, 0)');
  ctx.fillStyle = auraSecondary;
  ctx.fillRect(0, 0, width, height);
  ...
  const debtSeries = debtRaw.map((value) => Number((Math.max(0, value) * progress).toFixed(2)));

  const paidColor = options.paidColor || chartTheme.paidColor;
  const paidFill = options.paidFill || chartTheme.paidFill;
  const secondaryColor = options.debtColor || chartTheme.debtColor;
  const secondaryFill = options.debtFill || chartTheme.debtFill;
  const paidLabel = options.paidLabel || 'Pago';
  const secondaryLabel = options.debtLabel || (secondaryKey === 'charge' ? 'Cobrado' : 'Divida');
  const isNarrow = width <= 420;

  function drawLegendChip(x, y, color, text) {
    ctx.font = `${isNarrow ? 10 : 11}px ${CHART_HEADING_FONT}`;
    const chipHeight = isNarrow ? 24 : 28;
    const chipWidth = Math.max(84, Math.ceil(ctx.measureText(text).width + 34));
    drawRoundedRectPath(ctx, x, y, chipWidth, chipHeight, chipHeight / 2);
    ctx.fillStyle = chartTheme.pillBg;
    ctx.strokeStyle = chartTheme.pillBorder;
    ctx.lineWidth = 1;
    ctx.fill();
    ctx.stroke();
    ctx.beginPath();
    ctx.arc(x + 14, y + chipHeight / 2, 4.2, 0, Math.PI * 2);
    ctx.fillStyle = color;
    ctx.fill();
    ctx.fillStyle = chartTheme.label;
    ctx.textAlign = 'left';
    ctx.fillText(text, x + 24, y + chipHeight / 2 + 4);
    return chipWidth;
  }

  function drawTopTag(text) {
    ctx.font = `${isNarrow ? 9 : 10}px ${CHART_BODY_FONT}`;
    const tagWidth = Math.max(92, Math.ceil(ctx.measureText(text).width + 24));
    const tagX = width - tagWidth - 12;
    drawRoundedRectPath(ctx, tagX, 12, tagWidth, 24, 12);
    ctx.fillStyle = chartTheme.pillBg;
    ctx.strokeStyle = chartTheme.pillBorder;
    ctx.lineWidth = 1;
    ctx.fill();
    ...
    ctx.textAlign = 'left';
    ctx.fillText(text, tagX + 12, 28);
  }

  if (chartType === 'pie') {
    const paidTotal = paidSeries.reduce((sum, value) => sum + value, 0);
    const secondaryTotal = secondarySeries.reduce((sum, value) => sum + value, 0);
    const debtCurrent = Number(debtSeries[debtSeries.length - 1] || 0);
    const segments = [
      { label: `${paidLabel} 2026`, value: paidTotal, color: paidColor },
      { label: `${secondaryLabel} 2026`, value: secondaryTotal, color: secondaryColor },
      { label: 'Divida atual', value: debtCurrent, color: chartTheme.axisSoft }
    ].filter((segment) => segment.value > 0);

    if (segments.length === 0) {
      ctx.fillStyle = chartTheme.label;
      ctx.font = `700 ${isNarrow ? 13 : 15}px ${CHART_HEADING_FONT}`;
      ctx.textAlign = 'center';
      ctx.fillText(`Sem dados de ${FINANCE_CHART_YEAR}.`, width / 2, height / 2);
      return;
    }

    const total = segments.reduce((sum, segment) => sum + segment.value, 0);
    const centerX = width * (isNarrow ? 0.5 : 0.34);
    const centerY = height * 0.58;
    const radius = Math.min(width, height) * (isNarrow ? 0.24 : 0.22);
    ...
    ctx.fillStyle = chartTheme.bgTop;
    ctx.fill();

    ctx.fillStyle = chartTheme.label;
    ctx.font = `700 ${isNarrow ? 11 : 12}px ${CHART_HEADING_FONT}`;
    ctx.textAlign = 'center';
    ctx.fillText(`Resumo ${FINANCE_CHART_YEAR}`, centerX, centerY - 4);
    ctx.font = `700 ${isNarrow ? 16 : 20}px ${CHART_HEADING_FONT}`;
    ctx.fillText(formatCurrency(total), centerX, centerY + 18);

    drawTopTag(`Pizza ${FINANCE_CHART_YEAR}`);
    let legendY = isNarrow ? height - 96 : 64;
    const legendX = isNarrow ? 18 : width * 0.58;
    segments.forEach((segment) => {
      const percent = total > 0 ? ((segment.value / total) * 100).toFixed(1) : '0.0';
      drawLegendChip(legendX, legendY, segment.color, `${segment.label} ${percent}%`);
      legendY += isNarrow ? 34 : 38;
    });
    return;
  }

  const padding = isNarrow
    ? { left: 18, right: 58, top: 52, bottom: 34 }
    : { left: 24, right: 88, top: 58, bottom: 40 };
  const plotW = width - padding.left - padding.right;
  const plotH = height - padding.top - padding.bottom;
  if (plotW <= 40 || plotH <= 40) return;

  const rawMaxValue = Math.max(50, ...secondarySeries, ...paidSeries);
  const maxValue = (() => {
    const safeMax = Math.max(1, rawMaxValue * 1.08);
  ...
  for (let i = 0; i <= 4; i += 1) {
    ...
    ctx.lineTo(width - padding.right + 12, y);
    ctx.stroke();
    const tickValue = maxValue - (maxValue / 4) * i;
    ctx.fillStyle = chartTheme.axisSoft;
    ctx.font = `${isNarrow ? 9 : 10}px ${CHART_BODY_FONT}`;
    ctx.textAlign = 'right';
    ctx.fillText(formatCurrency(tickValue), width - 6, y + 3);
  }
  ctx.setLineDash([]);
  ctx.restore();
  ...
  if (chartType === 'bar') {
    const groupWidth = plotW / Math.max(pointsCount, 1);
    const barWidth = Math.max(8, Math.min(22, groupWidth * 0.28));
    labels.forEach((_, index) => {
      const centerX = padding.left + groupWidth * index + groupWidth / 2;
      const paidHeight = (paidSeries[index] / maxValue) * plotH;
      const secondaryHeight = (secondarySeries[index] / maxValue) * plotH;
      drawRoundedRectPath(ctx, centerX - barWidth - 3, padding.top + plotH - paidHeight, barWidth, Math.max(4, paidHeight), 8);
      ctx.fillStyle = paidColor;
      ctx.fill();
      drawRoundedRectPath(ctx, centerX + 3, padding.top + plotH - secondaryHeight, barWidth, Math.max(4, secondaryHeight), 8);
      ctx.fillStyle = secondaryColor;
      ctx.fill();
    });
    drawTopTag(`Barras ${FINANCE_CHART_YEAR}`);
  } else {
    ...
    function drawSmoothSeries(values, color, fillColor) {
      ...
      ctx.lineCap = 'round';
      ctx.lineJoin = 'round';
      ctx.shadowColor = chartTheme.shadow;
      ctx.shadowBlur = 16;
      ...
      values.forEach((value, index) => {
        ...
        ctx.beginPath();
        ctx.arc(x, y, index === values.length - 1 ? 5.2 : 3.1, 0, Math.PI * 2);
        ctx.fillStyle = color;
        ctx.fill();
      });
    }
    ...
    drawTopTag(`Linha ${FINANCE_CHART_YEAR}`);
  }

  const paidLegendWidth = drawLegendChip(padding.left, 12, paidColor, `${paidLabel} ${formatCurrency(paidSeries[paidSeries.length - 1] || 0)}`);
  let secondaryLegendX = padding.left + paidLegendWidth + 8;
  let secondaryLegendY = 12;
  if (secondaryLegendX > width - 160) {
    secondaryLegendX = padding.left;
    secondaryLegendY = isNarrow ? 40 : 44;
  }
  drawLegendChip(secondaryLegendX, secondaryLegendY, secondaryColor, `${secondaryLabel} ${formatCurrency(secondarySeries[secondarySeries.length - 1] || 0)}`);

  ctx.fillStyle = chartTheme.label;
  ctx.font = `${isNarrow ? 10 : 11}px ${CHART_BODY_FONT}`;
  ctx.textAlign = 'left';
  labels.forEach((label, index) => {
    if (index % labelStep !== 0 && index !== labels.length - 1) return;
    const x = chartType === 'bar'
      ? padding.left + (plotW / Math.max(pointsCount, 1)) * index + 4
      : pointX(index) - 10;
...
function drawWalletCanvas(canvas, user, optionsOrMonth = getCurrentMonthKey()) {
  ...
  const chartOptions = {
    ...
    paidColor: options.paidColor || chartTheme.paidColor,
    debtColor: options.debtColor || chartTheme.debtColor,
    paidFill: options.paidFill || chartTheme.paidFill,
    debtFill: options.debtFill || chartTheme.debtFill
  };
...
async function handleLogin() {
  ...
  try {
    resetInitialServerStateTracking();
    const payload = await apiRequest(API_AUTH_LOGIN_ENDPOINT, {
      method: 'POST',
      body: { username, password }
    });
    remoteSessionCreated = true;

    setLoginBusyState(true, 'Carregando...');
    const user = await activateAuthenticatedSession({
      username: payload && payload.user ? payload.user.username : username,
      role: payload && payload.user ? payload.user.role : 'member',
      loginAt: Number(payload && payload.createdAt) || Date.now()
    }, {
      registerAccess: true,
      showWelcome: true
    });
    ...
    if (remoteSessionCreated) {
      try {
        await apiRequest(API_AUTH_LOGOUT_ENDPOINT, { method: 'POST' });
      } catch {
        // ignore logout failures after partial login
      }
    }
    session = null;
    showNotification(error && error.message ? error.message : 'Falha ao autenticar.', 'error');
  } finally {
...
function handleLogout() {
  ...
  apiRequest(API_AUTH_LOGOUT_ENDPOINT, { method: 'POST' }).catch(() => {
...
function getNavigationLinks() {
  const links = [
    { id: 'dashboard', label: 'Painel' },
    { id: 'activityCenter', label: 'Central', badge: getActivityCenterBadgeCount() },
...
function buildNavInitials(label) {
  const parts = String(label || '')
    .trim()
    .split(/\s+/)
    .filter(Boolean)
    .slice(0, 2);
  return (parts.map((part) => part.charAt(0)).join('') || 'TP').toUpperCase();
}

function getNavigationMeta(pageId) {
  const meta = NAV_LINK_META[pageId];
  if (meta) return meta;
  return {
    group: 'workspace',
    icon: buildNavInitials(PAGE_TITLES[pageId] || pageId),
    hint: PAGE_TITLES[pageId] || 'Acesso rapido'
  };
}

function renderMobileQuickNav(links = getNavigationLinks()) {
  if (!els.mobileQuickNav) return;
  if (!session) {
    els.mobileQuickNav.innerHTML = '';
    return;
  }
  ...
  const quickLinks = MOBILE_QUICK_LINKS
    .map((item) => {
      const match = linksById.get(item.id);
      return match ? { ...match, mobileLabel: item.label } : null;
    })
    .filter(Boolean);
  ...
  quickLinks.forEach((link) => {
    const meta = getNavigationMeta(link.id);
    const button = document.createElement('button');
    button.type = 'button';
    button.className = 'mobile-quick-link';
    button.dataset.page = link.id;
    button.innerHTML = `
      <span class="mobile-quick-icon">${escapeHtml(meta.icon || buildNavInitials(link.label))}</span>
      <span class="mobile-quick-label">${escapeHtml(link.mobileLabel || link.label)}</span>
    `;
    button.addEventListener('click', () => {
      navigate(link.id);
    });
    els.mobileQuickNav.appendChild(button);
  });
}
...
function renderSidebar() {
  ...
  const scopedTickets = getScopedTickets();
  const pendingTickets = scopedTickets.filter((ticket) => ticket.status === 'pending').length;
  const openTasks = getUserTasks().filter((task) => task.status !== 'done').length;
  const onlineUsers = getOnlinePresenceList().length;
  const summaryName = sidebarUser ? getUserDisplayName(sidebarUser) : (session ? session.username : 'Equipe');
  const greetingName = String(summaryName || 'Equipe').split(/\s+/).filter(Boolean)[0] || 'Equipe';
  const summaryCard = document.createElement('section');
  summaryCard.className = 'sidebar-summary';
  summaryCard.innerHTML = `
    <div class="sidebar-summary-top">
      <span class="sidebar-brand-chip">TP Hub</span>
      <span class="sidebar-summary-role">${escapeHtml(roleLabel(sidebarUser ? sidebarUser.role : (session ? session.role : 'member')))}</span>
    </div>
    <div class="sidebar-summary-main">
      ${renderUserAvatar(sidebarUser || (session ? session.username : 'TP'), { size: 'md' })}
      <div class="sidebar-summary-copy">
        <strong>${escapeHtml(greetingName)}</strong>
        <span>Painel principal conectado</span>
      </div>
    </div>
    <div class="sidebar-summary-grid">
      <div><strong>${pendingTickets}</strong><span>Pendentes</span></div>
      <div><strong>${openTasks}</strong><span>Tarefas</span></div>
      <div><strong>${onlineUsers}</strong><span>Online</span></div>
    </div>
    <div class="sidebar-summary-actions">
      <button
        type="button"
        class="sidebar-action-btn"
        data-sidebar-action="theme"
        aria-pressed="${currentThemeMode === UI_THEME_DARK ? 'true' : 'false'}"
      >
        ${escapeHtml(currentThemeMode === UI_THEME_DARK ? 'Modo claro' : 'Modo escuro')}
      </button>
      <button type="button" class="sidebar-action-btn sidebar-action-btn-danger" data-sidebar-action="logout">Sair</button>
    </div>
  `;
  const sidebarThemeButton = summaryCard.querySelector('[data-sidebar-action="theme"]');
  if (sidebarThemeButton) {
    sidebarThemeButton.addEventListener('click', () => {
      toggleThemeMode();
      sidebarThemeButton.textContent = currentThemeMode === UI_THEME_DARK ? 'Modo claro' : 'Modo escuro';
      sidebarThemeButton.setAttribute('aria-pressed', currentThemeMode === UI_THEME_DARK ? 'true' : 'false');
    });
  }
  const sidebarLogoutButton = summaryCard.querySelector('[data-sidebar-action="logout"]');
  if (sidebarLogoutButton) {
    sidebarLogoutButton.addEventListener('click', handleLogout);
  }
  els.sidebar.appendChild(summaryCard);

  const links = getNavigationLinks();
  const linksByGroup = new Map();
  links.forEach((link) => {
    const meta = getNavigationMeta(link.id);
    if (!linksByGroup.has(meta.group)) {
      linksByGroup.set(meta.group, []);
    }
    linksByGroup.get(meta.group).push({ ...link, meta });
  });

  linksByGroup.forEach((groupLinks, groupId) => {
    const group = document.createElement('section');
    group.className = 'sidebar-group';
    const heading = document.createElement('p');
    heading.className = 'sidebar-group-title';
    heading.textContent = NAV_GROUP_TITLES[groupId] || 'Acessos';
    ...
    groupLinks.forEach((link) => {
      ...
      anchor.className = 'nav-link-card';
      const badgeHtml = typeof link.badge === 'number' && link.badge > 0
        ? `<span class="nav-pill">${link.badge}</span>`
        : '';
      anchor.innerHTML = `
        <span class="nav-link-main">
          <span class="nav-icon">${escapeHtml(link.meta.icon || buildNavInitials(link.label))}</span>
          <span class="nav-copy">
            <strong>${escapeHtml(link.label)}</strong>
            <small>${escapeHtml(link.meta.hint || PAGE_TITLES[link.id] || 'Acesso rapido')}</small>
          </span>
        </span>
        ${badgeHtml}
      `;
...
function updateSidebarSelection() {
  ...
  if (els.mobileQuickNav) {
    const quickLinks = els.mobileQuickNav.querySelectorAll('button[data-page]');
...
function renderActivityCenter() {
  if (!session) {
    renderAccessDenied('Faca login para abrir a central operacional.');
    return;
  }
  ...
  const attentionTickets = scopedTickets
    .filter((ticket) => ticket.priority === 'urgent' || ticket.status === 'pending')
    .slice(0, 6);
  const todayIsoDate = getTodayIsoDate();
  const tasksAtRisk = getUserTasks()
    .filter((task) => task.status !== 'done')
    .filter((task) => !task.dueDate || task.dueDate <= todayIsoDate)
    .sort((a, b) => String(a.dueDate || '9999-12-31').localeCompare(String(b.dueDate || '9999-12-31')))
    .slice(0, 6);
  const recentAnnouncements = announcements
    .slice()
    .filter((item) => (Number(item.deletedAt) || 0) === 0)
    .filter((item) => item.audience === 'all' || isAdminRole(session.role))
    .sort((a, b) => b.createdAt - a.createdAt)
    .slice(0, 4);
  const onlineUsers = getOnlinePresenceList().slice(0, 8);
  const recentOps = logs.slice().sort((a, b) => b.timestamp - a.timestamp).slice(0, 8);

  const attentionCards = attentionTickets.length > 0
    ? attentionTickets.map((ticket) => `
        <article class="result-card">
          <div class="result-card-head">
            <strong>Ticket #${ticket.id}</strong>
            <span class="result-tag">${escapeHtml(PRIORITY_LABELS[ticket.priority] || ticket.priority)}</span>
          </div>
          <p>${escapeHtml(toShortText(ticket.title, 120))}</p>
          <small>${escapeHtml(STATUS_LABELS[ticket.status] || ticket.status)} | ${escapeHtml(ticket.creator)}</small>
        </article>
      `).join('')
    : '<div class="empty-state">Nenhum ticket urgente ou pendente no seu escopo.</div>';

  const tasksCards = tasksAtRisk.length > 0
    ? tasksAtRisk.map((task) => `
        <article class="result-card">
          <div class="result-card-head">
            <strong>${escapeHtml(task.title)}</strong>
            <span class="result-tag">${escapeHtml(priorityLabel(task.priority))}</span>
          </div>
          <p>${escapeHtml(toShortText(task.description || 'Sem descricao adicional.', 120))}</p>
          <small>${task.dueDate ? `Entrega em ${escapeHtml(task.dueDate)}` : 'Sem prazo definido'}</small>
        </article>
      `).join('')
    : '<div class="empty-state">Nenhuma tarefa em risco imediato.</div>';

  const announcementCards = recentAnnouncements.length > 0
    ? recentAnnouncements.map((item) => `
        <article class="result-card">
          <div class="result-card-head">
            <strong>${escapeHtml(item.title)}</strong>
            <span class="result-tag">Comunicado</span>
          </div>
          <p>${escapeHtml(toShortText(item.content || '', 150))}</p>
          <small>${escapeHtml(item.author || 'sistema')} | ${formatDateTime(item.createdAt)}</small>
        </article>
      `).join('')
    : '<div class="empty-state">Sem comunicados recentes.</div>';

  const onlineRows = onlineUsers.length > 0
    ? onlineUsers.map((entry) => `
        <tr>
          <td>${renderUserIdentity(getUserByUsername(entry.username) || entry.username, { size: 'xs', showRole: false })}</td>
          <td>${escapeHtml(roleLabel(entry.role))}</td>
          <td>${escapeHtml(PAGE_TITLES[entry.page] || entry.page)}</td>
          <td>${escapeHtml(formatDuration(Math.max(0, Date.now() - entry.lastSeen)))}</td>
        </tr>
      `).join('')
    : '<tr><td colspan="4" class="empty-state">Ninguem online agora.</td></tr>';

  const recentOpsRows = recentOps.length > 0
    ? recentOps.map((entry) => `
        <tr>
      ...
      `).join('')
    : '<tr><td colspan="3" class="empty-state">Sem registros recentes.</td></tr>';

  els.content.innerHTML = `
    <section class="page-intro">
      <div class="page-intro-main">
        <span class="page-intro-kicker">Atencao imediata</span>
        <h2>Central operacional com o que precisa de decisao agora.</h2>
        <p>Uma leitura unica para enxergar gargalos, equipe online, comunicados e trilha recente sem navegar por varios modulos.</p>
      </div>
      <div class="page-intro-side">
        <div class="page-intro-stat"><strong>${attentionTickets.length}</strong><span>tickets criticos</span></div>
        <div class="page-intro-stat"><strong>${tasksAtRisk.length}</strong><span>tarefas em risco</span></div>
        <div class="page-intro-stat"><strong>${onlineUsers.length}</strong><span>pessoas online</span></div>
      </div>
    </section>
    ...
    <section class="panel">
      <div class="panel-header">
        <div>
          <h2 class="panel-title">Central operacional</h2>
          <p class="panel-subtitle">Visao rapida do que precisa de atencao agora, sem ficar pulando entre paginas.</p>
        </div>
        <div class="inline-actions">
          <button class="btn-primary" data-go="createTicket">Abrir ticket</button>
          <button class="btn-secondary" data-go="tasks">Ver tarefas</button>
          <button class="btn-ghost" data-go="announcements">Comunicados</button>
        </div>
      </div>
      <div class="stats-grid">
        <div class="stat-card">
          <div class="stat-value">${attentionTickets.length}</div>
          <div class="stat-label">Tickets criticos no radar</div>
        </div>
        <div class="stat-card">
          <div class="stat-value">${tasksAtRisk.length}</div>
          <div class="stat-label">Tarefas em risco hoje</div>
        </div>
        <div class="stat-card">
          <div class="stat-value">${onlineUsers.length}</div>
          <div class="stat-label">Pessoas online agora</div>
        </div>
        <div class="stat-card">
          <div class="stat-value">${recentAnnouncements.length}</div>
          <div class="stat-label">Comunicados ativos recentes</div>
        </div>
      </div>
    </section>
    ...
    <section class="panel">
      <div class="panel-header">
        <div>
          <h3 class="panel-title">Fila de atencao</h3>
          <p class="panel-subtitle">Tickets e tarefas que merecem resposta ou decisao primeiro.</p>
        </div>
      </div>
    ...
    <section class="panel">
      <div class="panel-header">
        <div>
          ...
          <p class="panel-subtitle">O que mudou ou precisa ser repassado para a operacao.</p>
        </div>
      </div>
      <div class="result-grid">
        ${announcementCards}
    ...
    <section class="panel">
      <div class="grid-2">
        <div>
          <h3 class="panel-title">Online agora</h3>
        ...
        <div>
          <h3 class="panel-title">Trilha recente</h3>
          <div class="table-wrap">
            <table>
              <thead>
                <tr>
  ...
  els.content.querySelectorAll('[data-go]').forEach((button) => {
    button.addEventListener('click', () => {
...
function renderSearchHub() {
  if (!session) {
    renderAccessDenied('Faca login para usar a busca global.');
    return;
  }

  const query = String(globalSearchQuery || (els.headerSearchInput ? els.headerSearchInput.value : '')).trim();
        ...
        ? results.map((item) => `
            <article class="result-card">
              ...
              <div class="inline-actions" style="margin-top: 12px;">
                <button class="btn-secondary" data-open-page="${escapeHtml(item.page)}">Abrir origem</button>
              </div>
            </article>
          `).join('')
        : '<div class="empty-state">Nenhum resultado para essa busca.</div>'
    )
    : '<div class="empty-state">Digite um termo acima para buscar usuarios, carteiras, tarefas, comunicados e logs.</div>';

  els.content.innerHTML = `
    <section class="page-intro page-intro-search">
      <div class="page-intro-main">
        <span class="page-intro-kicker">Busca inteligente</span>
        <h2>Encontre usuarios, carteiras, tarefas e registros em segundos.</h2>
        <p>Busca centralizada com resultados agrupados para cortar caminho na operacao diaria.</p>
      </div>
      <div class="page-intro-side">
        <div class="page-intro-stat"><strong>${results.length}</strong><span>resultados</span></div>
        <div class="page-intro-stat"><strong>${query ? '1' : '0'}</strong><span>consulta ativa</span></div>
        <div class="page-intro-stat"><strong>${query ? escapeHtml(query.slice(0, 12)) : '-'}</strong><span>termo atual</span></div>
      </div>
    </section>
    ...
    <section class="panel">
      <div class="panel-header">
        <div>
          <h2 class="panel-title">Busca global</h2>
          <p class="panel-subtitle">Procure por contexto operacional sem navegar pagina por pagina.</p>
        </div>
      </div>
      <div class="search-summary">
        <strong>Consulta:</strong> ${query ? escapeHtml(query) : 'nenhuma'}<br />
        <strong>Resultados:</strong> ${results.length}
      </div>
    </section>
  ...
  els.content.querySelectorAll('[data-open-page]').forEach((button) => {
    button.addEventListener('click', () => {
...
function renderDashboard() {
  ...
  const chartTheme = getActiveChartTheme();
  const financeChartType = getFinanceChartType();
  const financeChartScopeLabel = getFinanceChartScopeLabel();
  ...
  const financeHistory = user && Array.isArray(user.financeHistory)
    ? user.financeHistory
        .filter((entry) => isTimestampInFinanceChartYear(entry.timestamp))
        .slice()
  ...
  const heroName = String(getUserDisplayName(user) || user.username || 'Equipe').split(/\s+/).filter(Boolean)[0] || 'Equipe';
  const onlineUsers = getOnlinePresenceList().length;

  const heroStatCards = [
    { label: 'Pago no mes', value: formatCurrency(userMonthSummary.paid) },
    { label: 'Pendencias abertas', value: String(pendingCount) },
    { label: 'Tarefas ativas', value: String(openTasks.length) },
    { label: 'Equipe online', value: String(onlineUsers) }
  ];
  ...
  els.content.innerHTML = `
    <section class="dashboard-hero">
      <div class="dashboard-hero-main">
        <span class="hero-kicker">Operacao conectada em ${escapeHtml(monthLabel)}</span>
        <h2>Painel de ${escapeHtml(heroName)} com carteira, rotina e visao da TP em um lugar so.</h2>
        <p>Leitura rapida para acompanhar cobrancas, pagamentos, tarefas e movimento do time sem ficar pulando entre telas.</p>
        <div class="hero-chip-row">
          <span class="hero-chip">Lucro TP ${formatCurrency(aggregateMonth.profit)}</span>
          <span class="hero-chip">Divida atual ${formatDebtCurrency(user.debt)}</span>
          <span class="hero-chip">${formatCurrency(aggregateMonth.paid)} pagos na operacao</span>
        </div>
        <div class="hero-actions">
          <button id="dashboard-refresh" class="btn-primary" type="button">Atualizar painel</button>
          <button id="dashboard-open-central" class="btn-secondary" type="button">Abrir central</button>
          <button id="dashboard-open-search" class="btn-ghost" type="button">Busca global</button>
        </div>
      </div>
      <div class="dashboard-hero-side">
        <article class="hero-balance-card">
          <span>Carteira total da TP</span>
          <strong>${formatCurrency(aggregateMonth.walletValue)}</strong>
          <small>${formatCurrency(aggregateMonth.paid)} pagos no mes | ${formatCurrency(aggregateMonth.fines)} em cobrancas</small>
        </article>
    ...
    <section class="panel wallet-panel">
      <div class="panel-header">
        <div>
          ...
          <p class="panel-subtitle">Resumo consolidado com grafico claro e leitura direta somente em ${escapeHtml(financeChartScopeLabel)}.</p>
        </div>
        <div class="chart-toolbar">
          <label class="chart-toolbar-field" for="dashboard-chart-type">
            <span>Grafico</span>
            <select id="dashboard-chart-type">
              ${renderFinanceChartTypeOptions(financeChartType)}
            </select>
          </label>
        </div>
      </div>

      <div class="chart-period-note">Periodo exibido: <strong>${escapeHtml(financeChartScopeLabel)}</strong> | Ano ${FINANCE_CHART_YEAR}</div>
      ${aggregateChartHtml}
    ...
    <section class="panel wallet-panel">
      <div class="panel-header">
        <div>
          ...
          <p class="panel-subtitle">Painel pessoal organizado para pagar, simular e acompanhar evolucao somente em ${escapeHtml(financeChartScopeLabel)}.</p>
        </div>
      </div>
  ...
  const openCentralButton = document.getElementById('dashboard-open-central');
  const openSearchButton = document.getElementById('dashboard-open-search');
  ...
  if (canShowAggregateChart) {
    drawFinanceSeriesCanvasAnimated(totalCanvas, aggregateSeries, {
      ...
      paidColor: chartTheme.paidColor,
      debtColor: chartTheme.debtColor,
      paidFill: chartTheme.paidFill,
      debtFill: chartTheme.debtFill,
      durationMs: 860
    });
  }
  if (hasPersonalChart) {
    drawWalletCanvas(walletCanvas, user, {
      anchorMonthKey: selectedMonthKey,
      animate: true,
      paidLabel: 'Pago',
      debtLabel: 'Cobrado'
    });
  ...
  memberChartItems.forEach((entry) => {
    ...
    drawWalletCanvas(canvas, entry.member, {
      anchorMonthKey: selectedMonthKey,
      animate: true,
      paidLabel: 'Pago',
      debtLabel: 'Cobrado'
  ...
  if (openCentralButton) {
    openCentralButton.addEventListener('click', () => {
  ...
  if (chartTypeSelect) {
    chartTypeSelect.addEventListener('change', () => {
      setFinanceChartType(chartTypeSelect.value);
    });
  }
...
function renderWalletControl() {
    ...
    <section class="panel wallet-panel">
      <div class="panel-header">
        <div>
          ...
          <p class="panel-subtitle">Defina dividas, multas e valores entrando na carteira. Graficos filtrados apenas em ${escapeHtml(financeChartScopeLabel)}.</p>
        </div>
        <div class="inline-actions">
          <label class="chart-toolbar-field" for="wallet-control-chart-type">
            <span>Grafico</span>
            <select id="wallet-control-chart-type">
              ${renderFinanceChartTypeOptions(financeChartType)}
            </select>
          </label>
      ...
      <div class="chart-period-note">Periodo exibido nos graficos: <strong>${escapeHtml(financeChartScopeLabel)}</strong> | Ano ${FINANCE_CHART_YEAR}</div>
  ...
  const chartTypeSelect = document.getElementById('wallet-control-chart-type');
...
function renderUsersManagement() {
  ...
  form.addEventListener('submit', async (event) => {
    ...
    try {
      const result = await createUserOnServer(username, role, password);
      if (result && result.user) {
        users.push(normalizeUser(result.user));
      }
      showNotification('Usuario criado com sucesso.', 'success');
      form.reset();
      renderUsersManagement();
    } catch (error) {
      showNotification(error && error.message ? error.message : 'Falha ao criar usuario.', 'error');
    }
  });
  ...
  rows.addEventListener('click', (event) => {
    ...
    if (action === 'reset-pass') {
      ...
      resetUserPasswordOnServer(user.username, nextPassword.trim())
        .then(() => {
          user.passwordUpdatedAt = Date.now();
          showNotification('Senha atualizada.', 'success');
        })
        .catch((error) => {
          showNotification(error && error.message ? error.message : 'Falha ao atualizar senha.', 'error');
        });
      return;
    }
...
function renderSuperTools() {
  ...
  force1705Button.addEventListener('click', async () => {
    const now = Date.now();
    let affected = 0;
    for (const user of users) {
      try {
        await resetUserPasswordOnServer(user.username, DEFAULT_PASSWORD);
        user.passwordUpdatedAt = now;
        affected += 1;
      } catch {
        // ignore individual reset failures and keep going
      }
    }
    settings.passwordSeed1705Done = true;
    saveData();
    showNotification(`Senha 1705 aplicada para ${affected} usuarios.`, 'success');
  });
  ...
  resetIntelButton.addEventListener('click', () => {
    ...
    saveData();
    resetUserPasswordOnServer(username, DEFAULT_PASSWORD)
      .then(() => {
        intelUser.passwordUpdatedAt = Date.now();
        showNotification('Conta Inteligencia TP pronta (login: inteligencia tp / 1705).', 'success');
      })
      .catch((error) => {
        showNotification(error && error.message ? error.message : 'Falha ao resetar a conta Inteligencia TP.', 'error');
      });
  });
}
...
function renderProfile() {
  ...
  form.addEventListener('submit', async (event) => {
    ...
    try {
      await changeOwnPasswordOnServer(current, next);
      user.passwordUpdatedAt = Date.now();
      form.reset();
...
function bindStaticEvents() {
  ...
  if (els.loginThemeToggle) {
    els.loginThemeToggle.addEventListener('click', toggleThemeMode);
  }
  ...
  if (els.headerSearchButton) {
    els.headerSearchButton.addEventListener('click', () => {
      openGlobalSearch();
    });
  }
  if (els.headerSearchInput) {
    els.headerSearchInput.addEventListener('keydown', (event) => {
      if (event.key === 'Enter') {
        event.preventDefault();
        openGlobalSearch();
      }
  ...
  window.addEventListener('storage', (event) => {
    if (event.key === STORAGE_KEYS.ui) {
      const uiState = readUiState();
      if (!session && typeof uiState.lastUser === 'string' && document.activeElement !== els.usernameInput) {
        els.usernameInput.value = uiState.lastUser;
      }
      applyThemeMode(uiState.theme || UI_THEME_LIGHT, { persist: false, rerender: Boolean(session) });
    }
...
var WALLET_REDESIGN_VERSION = 20260331;
var WALLET_INITIAL_DATA_YEAR = 2026;
var WALLET_RECORD_STATUSES = ['open', 'partial', 'paid', 'scheduled'];
var WALLET_RECORD_TYPES = ['payment', 'debt', 'fine', 'loan', 'adjustment'];
var WALLET_SEED_DATA = [
  {
    username: 'felps',
    displayName: 'Felps',
    role: 'member',
    payments: { 1: 900, 2: 900, 3: 700 },
    debts: { 1: 185, 2: 185, 3: 200 }
  },
  {
    username: 'matheus',
    displayName: 'Matheus',
    role: 'member',
    payments: { 1: 569.75, 2: 569.75, 3: 314.5 },
    debts: { 1: 185, 2: 185, 3: 200 }
  },
  {
    username: 'murilo',
    displayName: 'Murilo',
    role: 'member',
    payments: { 1: 75, 2: 75, 3: 66.01 },
    debts: { 1: 100, 2: 100, 3: 100 }
  },
  {
    username: 'yoon',
    displayName: 'Yoon',
    role: 'member',
    payments: { 1: 70, 2: 70, 3: 73 },
    debts: { 1: 70, 2: 70, 3: 70 }
  },
  {
    username: 'lia',
    displayName: 'Lia',
    role: 'member',
    payments: { 1: 101.5, 2: 101.5, 3: 0 },
    debts: { 1: 100, 2: 100, 3: 100 }
  }
];
var walletDetailState = {
  username: '',
  year: WALLET_INITIAL_DATA_YEAR,
  editingRecordId: null
};

Object.assign(PAGE_TITLES, {
  stealthChat: 'Chat Dispiagem',
  intelCenter: 'Central Intel',
  walletControl: 'Carteiras'
});

Object.assign(NAV_LINK_META, {
  dashboard: { group: 'overview', icon: 'PG', hint: 'Carteiras, historico e visao geral' },
  activityCenter: { group: 'overview', icon: 'CT', hint: 'Central limpa para prioridades reais' },
  stealthChat: { group: 'workspace', icon: 'CD', hint: 'Chat Dispiagem preservado' },
  staffChat: { group: 'workspace', icon: 'CI', hint: 'Chat interno da equipe' },
  intelCenter: { group: 'control', icon: 'IT', hint: 'Radar financeiro e leitura estrategica' },
  walletControl: { group: 'control', icon: 'CR', hint: 'Admin completo da carteira' }
});
...
function normalizeFinanceRecordStatus(value, fallback = '') {
  const normalized = String(value || '').trim().toLowerCase();
  if (!normalized) return fallback;
  return WALLET_RECORD_STATUSES.includes(normalized) ? normalized : fallback;
}

function normalizeFinanceRecordType(value, fallback = 'adjustment') {
  const normalized = String(value || '').trim().toLowerCase();
  if (!normalized) return fallback;
  return WALLET_RECORD_TYPES.includes(normalized) ? normalized : fallback;
}

function inferFinanceRecordType(type, note = '', options = {}) {
  const explicit = normalizeFinanceRecordType(options.recordType, '');
  ...
  if (/multa|juros|atraso/.test(normalizedNote)) return 'fine';
  return 'debt';
}
...
function getMonthLongLabelFromKey(monthKey) {
...
function getMonthShortLabelFromKey(monthKey) {
  const parsed = parseMonthKey(monthKey);
  if (!parsed) return String(monthKey || '-');
  return new Date(parsed.year, parsed.month - 1, 1).toLocaleDateString('pt-BR', { month: 'short' });
}

function getFinanceEntryMonthRef(entry) {
  const explicit = parseMonthKey(entry && entry.monthRef) ? entry.monthRef : '';
  return explicit || getMonthKeyFromTimestamp(entry && entry.timestamp);
}

function getFinanceEntryYear(entry) {
  const parsed = parseMonthKey(getFinanceEntryMonthRef(entry));
  return parsed ? parsed.year : FINANCE_CHART_YEAR;
}

function normalizeFinanceHistoryEntry(entry) {
  const source = entry && typeof entry === 'object' && !Array.isArray(entry) ? entry : {};
  const timestamp = Number(source.timestamp) || Date.now();
  const note = String(source.note || '').trim();
  const details = String(source.details || '').trim();
  return {
    id: Number(source.id) || createId(),
    type: ['charge', 'payment', 'loan', 'adjustment'].includes(source.type) ? source.type : 'adjustment',
    amount: roundCurrencyValue(Math.max(0, Number(source.amount) || 0)),
    note: note.slice(0, 200),
    actor: String(source.actor || 'sistema').trim().slice(0, 80) || 'sistema',
    timestamp,
    updatedAt: Math.max(timestamp, Number(source.updatedAt) || 0),
    monthRef: parseMonthKey(source.monthRef) ? String(source.monthRef) : getMonthKeyFromTimestamp(timestamp),
    recordType: normalizeFinanceRecordType(source.recordType, inferFinanceRecordType(source.type, note, source)),
    details: details.slice(0, 240),
    status: normalizeFinanceRecordStatus(source.status, ''),
    seedKey: String(source.seedKey || '').trim().slice(0, 80)
  };
}

function normalizeFinanceHistoryCollection(rawHistory) {
  const source = Array.isArray(rawHistory) ? rawHistory : [];
  return source
    .map(normalizeFinanceHistoryEntry)
    .filter((entry) => entry.amount > 0)
    .sort((a, b) => {
...
function isFinancePaymentEntry(entry) {
  return entry && entry.type === 'payment';
}

function isFinanceDebtEntry(entry) {
  return Boolean(entry) && entry.type !== 'payment';
}

function recalculateUserFinanceSnapshot(user) {
  if (!user) return user;
  const history = normalizeFinanceHistoryCollection(user.financeHistory);
  let debt = 0;
  let totalCharged = 0;
  let totalPaid = 0;
  let walletProfit = 0;
  let emergencyLoanOutstanding = 0;

  history.forEach((entry) => {
    const amount = roundCurrencyValue(entry.amount);
    if (amount <= 0) return;

    if (isFinancePaymentEntry(entry)) {
      const appliedToDebt = Math.min(amount, debt);
      const profitAdded = Math.max(0, amount - appliedToDebt);
      debt = roundCurrencyValue(Math.max(0, debt - appliedToDebt));
      totalPaid = roundCurrencyValue(totalPaid + amount);
      walletProfit = roundCurrencyValue(walletProfit + profitAdded);
      emergencyLoanOutstanding = roundCurrencyValue(Math.max(0, emergencyLoanOutstanding - appliedToDebt));
      return;
    }

    debt = roundCurrencyValue(debt + amount);
    totalCharged = roundCurrencyValue(totalCharged + amount);
    if (entry.type === 'loan' || entry.recordType === 'loan') {
      emergencyLoanOutstanding = roundCurrencyValue(emergencyLoanOutstanding + amount);
    }
  });

  const financeUpdatedAt = Math.max(
    Number(user.financeUpdatedAt) || 0,
    ...history.map((entry) => Math.max(Number(entry.timestamp) || 0, Number(entry.updatedAt) || 0))
  );
  user.financeHistory = history;
  user.debt = roundCurrencyValue(debt);
  user.totalCharged = roundCurrencyValue(totalCharged);
  user.totalPaid = roundCurrencyValue(totalPaid);
  user.walletProfit = roundCurrencyValue(walletProfit);
  user.emergencyLoanOutstanding = roundCurrencyValue(emergencyLoanOutstanding);
  user.financeUpdatedAt = financeUpdatedAt;
  return user;
}

function buildUserDebtSettlementMap(user) {
  const history = normalizeFinanceHistoryCollection(user && user.financeHistory);
  const debtQueue = [];
  const settlement = new Map();

  history.forEach((entry) => {
    if (isFinanceDebtEntry(entry)) {
      const tracker = {
        id: entry.id,
        amount: roundCurrencyValue(entry.amount),
        remaining: roundCurrencyValue(entry.amount),
        settledAt: 0
      };
      debtQueue.push(tracker);
      settlement.set(entry.id, tracker);
      return;
    }

    let available = roundCurrencyValue(entry.amount);
    debtQueue.forEach((tracker) => {
      if (available <= 0 || tracker.remaining <= 0) return;
      const applied = Math.min(tracker.remaining, available);
      tracker.remaining = roundCurrencyValue(tracker.remaining - applied);
      available = roundCurrencyValue(available - applied);
      if (tracker.remaining <= 0 && !tracker.settledAt) {
        tracker.settledAt = entry.timestamp;
      }
    });
  });
...
function getFinanceEntryDisplayStatus(entry, settlementMap) {
  const manualStatus = normalizeFinanceRecordStatus(entry && entry.status, '');
  if (manualStatus) return manualStatus;
  if (isFinancePaymentEntry(entry)) return 'paid';
  const tracker = settlementMap && settlementMap.get(entry.id);
  if (!tracker) return 'open';
  if (tracker.remaining <= 0) return 'paid';
  if (tracker.remaining < tracker.amount) return 'partial';
  return 'open';
}

function getFinanceEntryRemainingAmount(entry, settlementMap) {
  const tracker = settlementMap && settlementMap.get(entry.id);
  return tracker ? roundCurrencyValue(tracker.remaining) : 0;
}

function getFinanceHistoryYears(user) {
  const years = new Set([FINANCE_CHART_YEAR]);
  normalizeFinanceHistoryCollection(user && user.financeHistory).forEach((entry) => {
    years.add(getFinanceEntryYear(entry));
  });
  return Array.from(years).sort((a, b) => b - a);
}

function getFinanceYearTimeline(user, year = FINANCE_CHART_YEAR) {
  const safeYear = Number(year) || FINANCE_CHART_YEAR;
  const monthKeys = getFinanceChartMonthKeys(safeYear);
  const settlementMap = buildUserDebtSettlementMap(user);
  const monthMap = new Map();
  const timeline = monthKeys.map((monthKey) => {
    const month = {
      key: monthKey,
      label: getMonthLongLabelFromKey(monthKey),
      shortLabel: getMonthShortLabelFromKey(monthKey),
      payments: [],
      debts: [],
      paidTotal: 0,
      debtTotal: 0,
  ...
  normalizeFinanceHistoryCollection(user && user.financeHistory).forEach((entry) => {
    const monthKey = getFinanceEntryMonthRef(entry);
    const month = monthMap.get(monthKey);
    if (!month) return;

    const descriptor = {
      ...entry,
      displayStatus: getFinanceEntryDisplayStatus(entry, settlementMap),
      remainingAmount: getFinanceEntryRemainingAmount(entry, settlementMap),
      settledAt: settlementMap && settlementMap.get(entry.id) ? settlementMap.get(entry.id).settledAt : 0,
      monthRef: monthKey
    };
    if (isFinancePaymentEntry(entry)) {
      month.payments.push(descriptor);
      month.paidTotal = roundCurrencyValue(month.paidTotal + entry.amount);
    } else {
      month.debts.push(descriptor);
      month.debtTotal = roundCurrencyValue(month.debtTotal + entry.amount);
    }
  });

  timeline.forEach((month) => {
    month.payments.sort((a, b) => b.timestamp - a.timestamp);
    month.debts.sort((a, b) => b.timestamp - a.timestamp);
    month.totalRecords = month.payments.length + month.debts.length;
    if (month.totalRecords === 0) {
      month.status = 'scheduled';
      return;
    }
    if (month.debts.length === 0) {
      month.status = 'paid';
      return;
    }
    const statuses = month.debts.map((entry) => entry.displayStatus);
...
function getFinanceYearTotals(user, year = FINANCE_CHART_YEAR) {
  const timeline = getFinanceYearTimeline(user, year);
  return {
    paidYear: roundCurrencyValue(timeline.reduce((sum, month) => sum + month.paidTotal, 0)),
    debtYear: roundCurrencyValue(timeline.reduce((sum, month) => sum + month.debtTotal, 0)),
    accumulatedTotal: roundCurrencyValue(Number(user && user.totalPaid) || 0),
    openDebt: roundCurrencyValue(Number(user && user.debt) || 0),
    totalRecords: timeline.reduce((sum, month) => sum + month.totalRecords, 0),
    generalStatus: Math.max(0, Number(user && user.debt) || 0) <= 0 ? 'Em dia' : 'Com pendencias'
  };
}

function createWalletSeedTimestamp(year, month, day, hour) {
  return new Date(year, month - 1, day, hour, 0, 0, 0).getTime();
}

function appendWalletSeedYearRecords(user, seed, year) {
  if (!user || !seed) return false;
  const currentHistory = normalizeFinanceHistoryCollection(user.financeHistory);
  const alreadySeeded = currentHistory.some((entry) => entry.seedKey && entry.seedKey.startsWith(`wallet-seed-${year}-`));
  const hasYearHistory = currentHistory.some((entry) => getFinanceEntryYear(entry) === year);
  if (alreadySeeded || hasYearHistory) return false;

  const nextHistory = currentHistory.slice();
  for (let month = 1; month <= 3; month += 1) {
    const monthKey = getMonthKeyFromParts(year, month);
    const monthLabel = getMonthLongLabelFromKey(monthKey);
    const debtAmount = roundCurrencyValue(seed.debts[month] || 0);
    const paymentAmount = roundCurrencyValue(seed.payments[month] || 0);
    if (debtAmount > 0) {
      nextHistory.push(normalizeFinanceHistoryEntry({
        id: createId(),
        type: 'charge',
        amount: debtAmount,
        note: `Divida de ${monthLabel}`,
        details: 'Divida mensal registrada no historico inicial.',
        actor: 'sistema',
        timestamp: createWalletSeedTimestamp(year, month, 5, 10),
        monthRef: monthKey,
        recordType: 'debt',
        status: 'paid',
        seedKey: `wallet-seed-${year}-${seed.username}-debt-${month}`
      }));
    }
    if (paymentAmount > 0 || month === 3) {
      nextHistory.push(normalizeFinanceHistoryEntry({
        id: createId(),
        type: 'payment',
        amount: paymentAmount,
        note: `Pagamento de ${monthLabel}`,
        details: month === 3
          ? 'Pagamento do mes informado no cadastro inicial.'
          : 'Distribuicao inicial automatica do total acumulado.',
        actor: 'sistema',
        timestamp: createWalletSeedTimestamp(year, month, new Date(year, month, 0).getDate(), 18),
        monthRef: monthKey,
        recordType: 'payment',
        status: 'paid',
        seedKey: `wallet-seed-${year}-${seed.username}-payment-${month}`
      }));
    }
  }

  user.financeHistory = nextHistory;
  recalculateUserFinanceSnapshot(user);
  return true;
}

function ensureWalletSeedUser(seed) {
  let user = getUserByUsername(seed.username);
  if (!user) {
    user = normalizeUser({
      username: seed.username,
  ...
  if (normalizeDisplayName(user.displayName, user.username) !== seed.displayName) {
    user.displayName = seed.displayName;
    user.profileUpdatedAt = Date.now();
  }
  if (canRoleReceiveWalletChart(user.role) && user.walletChartEnabled !== true) {
    user.walletChartEnabled = true;
    user.walletChartUpdatedAt = Date.now();
  }
  return user;
...
function runWalletRedesignMigration() {
  if (!settings || settings.walletRedesignVersion === WALLET_REDESIGN_VERSION) return false;

  let changed = false;
  WALLET_SEED_DATA.forEach((seed) => {
    const user = ensureWalletSeedUser(seed);
    if (appendWalletSeedYearRecords(user, seed, WALLET_INITIAL_DATA_YEAR)) {
      changed = true;
    }
    recalculateUserFinanceSnapshot(user);
  });

  settings.walletRedesignVersion = WALLET_REDESIGN_VERSION;
  settings.walletSeedYear = WALLET_INITIAL_DATA_YEAR;
  if (changed) {
    addLog(`Aplicou migracao premium da carteira ${WALLET_INITIAL_DATA_YEAR} com historico mensal detalhado`);
  }
  saveData({ immediate: true });
  return changed;
}
...
function getThemeMetaColor(mode = currentThemeMode) {
  return mode === UI_THEME_DARK ? '#08181b' : '#f3f2ec';
}

function getActiveChartTheme() {
  return currentThemeMode === UI_THEME_DARK
    ? {
        paidColor: '#58d6b2',
        paidFill: 'rgba(88, 214, 178, 0.22)',
        debtColor: '#f0a35a',
        debtFill: 'rgba(240, 163, 90, 0.22)',
        bgTop: 'rgba(12, 22, 24, 0.98)',
        bgBottom: 'rgba(7, 13, 15, 0.99)',
        grid: 'rgba(127, 170, 168, 0.12)',
        label: '#f2f4ef',
        axis: '#d7efe5',
        axisSoft: '#8ab6ae',
        orbA: 'rgba(88, 214, 178, 0.18)',
        orbB: 'rgba(240, 163, 90, 0.16)',
        shadow: 'rgba(1, 6, 7, 0.48)',
        pillBg: 'rgba(20, 35, 38, 0.92)',
        pillBorder: 'rgba(120, 165, 160, 0.16)'
      }
    : {
        paidColor: '#0f9b77',
        paidFill: 'rgba(15, 155, 119, 0.16)',
        debtColor: '#d47d2a',
        debtFill: 'rgba(212, 125, 42, 0.16)',
        bgTop: 'rgba(255, 255, 252, 0.98)',
        bgBottom: 'rgba(244, 240, 230, 0.98)',
        grid: 'rgba(71, 112, 106, 0.12)',
        label: '#173a3b',
        axis: '#235657',
        axisSoft: '#557a77',
        orbA: 'rgba(15, 155, 119, 0.14)',
        orbB: 'rgba(212, 125, 42, 0.12)',
        shadow: 'rgba(31, 56, 50, 0.18)',
        pillBg: 'rgba(255, 255, 255, 0.88)',
        pillBorder: 'rgba(70, 110, 105, 0.12)'
      };
}
...
function updateClock() {
  ...
  if (FINANCE_CHART_YEAR !== nextYear) {
    FINANCE_CHART_YEAR = nextYear;
    if (session) requestRealtimeRender();
  }
  if (!els.liveClock) return;
  els.liveClock.textContent = now.toLocaleString('pt-BR');
}
...
function normalizeUser(user) {
  ...
  const fallbackAvatarUpdatedAt = hasRawAvatarField
    ? Math.max(0, Number(source.profileUpdatedAt) || 0, avatarDataUrl ? 1 : 0)
    : (avatarDataUrl ? Math.max(1, Number(source.profileUpdatedAt) || 0) : 0);
  const financeHistory = normalizeFinanceHistoryCollection(source.financeHistory);
  const financeUpdatedAt = Math.max(
    Math.max(0, Number(source.financeUpdatedAt) || 0),
    ...financeHistory.map((entry) => Math.max(Number(entry.timestamp) || 0, Number(entry.updatedAt) || 0))
  );
  const walletAccessEnabled = normalizedRole === 'member'
    ? true
    : (normalizedRole === 'admin'
  ...
  return recalculateUserFinanceSnapshot(normalizedUser);
}
...
function registerFinanceEvent(user, type, amount, note = '', actor = null, options = {}) {
  ...
  const eventTimestamp = Number(options.timestamp) > 0 ? Number(options.timestamp) : Date.now();
  const eventType = ['charge', 'payment', 'loan', 'adjustment'].includes(type) ? type : 'adjustment';
  const previousDebt = Math.max(0, Number(user.debt) || 0);
  const appliedToDebt = eventType === 'payment' ? Math.min(safeAmount, previousDebt) : 0;
  const profitAdded = eventType === 'payment' ? Math.max(0, safeAmount - appliedToDebt) : 0;

  user.financeHistory = normalizeFinanceHistoryCollection([
    ...(Array.isArray(user.financeHistory) ? user.financeHistory : []),
    {
      id: createId(),
      type: eventType,
      amount: safeAmount,
      note: String(note || ''),
      actor: actor || (session ? session.username : 'sistema'),
      timestamp: eventTimestamp,
      updatedAt: Date.now(),
      monthRef: options.monthRef || getMonthKeyFromTimestamp(eventTimestamp),
      recordType: inferFinanceRecordType(eventType, note, options),
      details: String(options.details || ''),
      status: normalizeFinanceRecordStatus(options.status, eventType === 'payment' ? 'paid' : '')
    }
  ]);
  recalculateUserFinanceSnapshot(user);
  user.financeUpdatedAt = Math.max(Number(user.financeUpdatedAt) || 0, Date.now(), eventTimestamp);

  if (options.skipSave !== true) saveData();
  return {
    ok: true,
    amount: roundCurrencyValue(safeAmount),
    appliedToDebt: roundCurrencyValue(appliedToDebt),
    profitAdded: roundCurrencyValue(profitAdded),
    timestamp: eventTimestamp,
    message: profitAdded > 0 ? 'Pagamento com lucro registrado.' : 'Operacao registrada.'
  };
}
...
function getFinanceChartMonthKeys(year = FINANCE_CHART_YEAR) {
  ...
  for (let month = 1; month <= 12; month += 1) {
    keys.push(getMonthKeyFromParts(safeYear, month));
  }
  return keys;
}
...
function isTimestampInFinanceChartYear(timestamp, year = FINANCE_CHART_YEAR) {
  const date = new Date(Number(timestamp) || 0);
  return date.getFullYear() === Number(year || FINANCE_CHART_YEAR);
}

function getFinanceChartScopeLabel() {
  return `Janeiro a Dezembro de ${FINANCE_CHART_YEAR}`;
}

function getUserFinanceMonthSummary(user, monthKey) {
  const safeMonthKey = parseMonthKey(monthKey) ? monthKey : getCurrentMonthKey();
  const parsed = parseMonthKey(safeMonthKey) || parseMonthKey(getCurrentMonthKey());
  const month = getFinanceYearTimeline(user, parsed.year).find((entry) => entry.key === safeMonthKey);
  const debtEntries = month ? month.debts : [];
  return {
    charged: roundCurrencyValue(month ? month.debtTotal : 0),
    paid: roundCurrencyValue(month ? month.paidTotal : 0),
    loans: roundCurrencyValue(debtEntries.filter((entry) => entry.recordType === 'loan').reduce((sum, entry) => sum + entry.amount, 0)),
    fines: roundCurrencyValue(debtEntries.filter((entry) => entry.recordType === 'fine').reduce((sum, entry) => sum + entry.amount, 0)),
    debt: roundCurrencyValue(Number(user && user.debt) || 0),
    walletValue: roundCurrencyValue(Number(user && user.totalPaid) || 0),
    profit: roundCurrencyValue(Number(user && user.walletProfit) || 0)
  };
}
...
function getAggregateMonthFinanceSummary(monthKey, scopedUsers = users) {
  ...
  scopedUsers.forEach((user) => {
    ...
    totals.loans += summary.loans;
    totals.fines += summary.fines;
    totals.debt += summary.debt;
    totals.walletValue += summary.walletValue;
    totals.profit += summary.profit;
  });

  return {
    charged: roundCurrencyValue(totals.charged),
    paid: roundCurrencyValue(totals.paid),
    loans: roundCurrencyValue(totals.loans),
    fines: roundCurrencyValue(totals.fines),
    debt: roundCurrencyValue(totals.debt),
    walletValue: roundCurrencyValue(totals.walletValue),
    profit: roundCurrencyValue(totals.profit)
  };
}

function buildFinanceSeries(user, monthsCount = 12, anchorMonthKey = getCurrentMonthKey()) {
  const parsedAnchor = parseMonthKey(anchorMonthKey) || parseMonthKey(getCurrentMonthKey());
  const year = parsedAnchor ? parsedAnchor.year : FINANCE_CHART_YEAR;
  const monthKeys = getFinanceChartMonthKeys(year);
  const monthSet = new Set(monthKeys);
  const months = monthKeys.map((key) => ({
    key,
    label: getMonthShortLabelFromKey(key),
    charges: 0,
    payments: 0
  }));
  const monthIndex = new Map(months.map((month, index) => [month.key, index]));
  ...
  history.forEach((entry) => {
    ...
    const amount = roundCurrencyValue(entry.amount);
    if (amount <= 0) return;
    if (!monthSet.has(monthKey) && monthKey < monthKeys[0]) {
      if (isFinancePaymentEntry(entry)) {
        debtLevel = roundCurrencyValue(Math.max(0, debtLevel - amount));
      } else {
        debtLevel = roundCurrencyValue(debtLevel + amount);
      }
      return;
    }
    ...
    if (isFinancePaymentEntry(entry)) {
      months[idx].payments = roundCurrencyValue(months[idx].payments + amount);
    } else {
      months[idx].charges = roundCurrencyValue(months[idx].charges + amount);
    }
  });
  ...
  const labels = [];
  months.forEach((month) => {
    debtLevel = roundCurrencyValue(Math.max(0, debtLevel + month.charges - month.payments));
    debtSeries.push(roundCurrencyValue(debtLevel));
    paidSeries.push(roundCurrencyValue(month.payments));
    chargeSeries.push(roundCurrencyValue(month.charges));
    labels.push(month.label);
  });

  return { labels, debtSeries, paidSeries, chargeSeries, monthKeys, year };
}

function buildAggregateFinanceSeries(monthsCount = 12, anchorMonthKey = getCurrentMonthKey(), scopedUsers = users) {
  const parsedAnchor = parseMonthKey(anchorMonthKey) || parseMonthKey(getCurrentMonthKey());
  const year = parsedAnchor ? parsedAnchor.year : FINANCE_CHART_YEAR;
  const monthKeys = getFinanceChartMonthKeys(year);
  const labels = monthKeys.map(getMonthShortLabelFromKey);
  const debtSeries = new Array(monthKeys.length).fill(0);
  const paidSeries = new Array(monthKeys.length).fill(0);
  const chargeSeries = new Array(monthKeys.length).fill(0);

  scopedUsers.forEach((user) => {
    const userSeries = buildFinanceSeries(user, 12, getMonthKeyFromParts(year, 12));
    userSeries.debtSeries.forEach((value, index) => {
      debtSeries[index] += Number(value) || 0;
    });
    userSeries.paidSeries.forEach((value, index) => {
  ...
  return {
    monthKeys,
    labels,
    debtSeries: debtSeries.map(roundCurrencyValue),
    paidSeries: paidSeries.map(roundCurrencyValue),
    chargeSeries: chargeSeries.map(roundCurrencyValue),
    year
  };
}
...
function getFinanceRecordTypeLabel(recordType) {
  if (recordType === 'payment') return 'Pagamento';
  if (recordType === 'fine') return 'Multa/Juros';
  if (recordType === 'loan') return 'Emprestimo';
  if (recordType === 'adjustment') return 'Ajuste';
  return 'Divida';
}
...
function parseDateTimeInputValue(value) {
  ...
  return parseFlexibleDateTime(normalized) || 0;
}

function getRecentWalletEvents(limit = 8, scopedUsers = users) {
  const entries = [];
  scopedUsers.forEach((user) => {
    normalizeFinanceHistoryCollection(user.financeHistory).forEach((entry) => {
      entries.push({
        user,
        entry,
        monthRef: getFinanceEntryMonthRef(entry)
      });
    });
  });
  return entries
...
function ensureWalletDetailShell() {
  let modal = document.getElementById('wallet-detail-modal');
  if (modal) return modal;

  modal = document.createElement('div');
  modal.id = 'wallet-detail-modal';
  modal.className = 'wallet-detail-modal hidden';
  modal.innerHTML = `
    <div class="wallet-detail-backdrop" data-wallet-close="true"></div>
    <div class="wallet-detail-panel">
      <div id="wallet-detail-content" class="wallet-detail-content"></div>
    </div>
  `;
  document.body.appendChild(modal);

  modal.addEventListener('click', (event) => {
    const closeTarget = event.target.closest('[data-wallet-close="true"]');
    if (closeTarget) {
...
function closeWalletDetailsModal() {
  ...
  walletDetailState.editingRecordId = null;
}

function rerenderCurrentPagePreservingWalletModal() {
  if (!session) return;
  renderContent(currentPage);
  renderSidebar();
  if (walletDetailState.username) {
    renderWalletDetailsModal();
  }
}

function getWalletEditableRecord(user, recordId) {
  return normalizeFinanceHistoryCollection(user && user.financeHistory).find((entry) => Number(entry.id) === Number(recordId)) || null;
}

function renderWalletRecordList(records, tone = 'payment') {
  if (!records.length) {
    return '<div class="wallet-record-empty">Sem registros neste bloco.</div>';
  }
  return records.map((record) => `
    <article class="wallet-record-item ${tone}">
      <div class="wallet-record-main">
        <div>
          <strong>${getFinanceRecordTypeLabel(record.recordType)} ${formatCurrency(record.amount)}</strong>
          <p>${escapeHtml(record.details || record.note || 'Sem observacao detalhada.')}</p>
        </div>
        <span class="wallet-status-badge ${escapeHtml(record.displayStatus)}">${escapeHtml(getWalletStatusLabel(record.displayStatus))}</span>
      </div>
      <div class="wallet-record-meta">
        <span>Data: <strong>${escapeHtml(formatDateTime(record.timestamp))}</strong></span>
        <span>Referencia: <strong>${escapeHtml(getMonthLongLabelFromKey(record.monthRef))}</strong></span>
        ${tone === 'debt' ? `<span>Restante: <strong>${formatCurrency(record.remainingAmount)}</strong></span>` : ''}
        ${record.settledAt ? `<span>Quitado em: <strong>${escapeHtml(formatDateTime(record.settledAt))}</strong></span>` : ''}
      </div>
      ${canManageWallets() ? `
        <div class="wallet-record-actions">
          <button class="btn-ghost" type="button" data-wallet-record-edit="${record.id}">Editar</button>
          <button class="btn-danger" type="button" data-wallet-record-delete="${record.id}">Excluir</button>
        </div>
      ` : ''}
    </article>
  `).join('');
...
function bindWalletDetailTriggers(root = document) {
  root.querySelectorAll('[data-wallet-open]').forEach((button) => {
    button.addEventListener('click', () => {
      openWalletDetailsModal(button.dataset.walletOpen, Number(button.dataset.walletYear) || FINANCE_CHART_YEAR);
    });
  });
}

function openWalletDetailsModal(username, year = FINANCE_CHART_YEAR) {
  const user = getUserByUsername(username);
  if (!user) {
    showNotification('Carteira nao encontrada.', 'warning');
    return;
  }
  walletDetailState.username = user.username;
  walletDetailState.year = Number(year) || FINANCE_CHART_YEAR;
  walletDetailState.editingRecordId = null;
  renderWalletDetailsModal();
  const modal = ensureWalletDetailShell();
  modal.classList.remove('hidden');
  requestAnimationFrame(() => modal.classList.add('open'));
}

function renderWalletDetailsModal() {
  const modal = ensureWalletDetailShell();
  const content = document.getElementById('wallet-detail-content');
  if (!content) return;
  const user = getUserByUsername(walletDetailState.username);
  if (!user) {
    closeWalletDetailsModal();
    return;
  }

  const availableYears = getFinanceHistoryYears(user);
  if (!availableYears.includes(walletDetailState.year)) {
    walletDetailState.year = availableYears[0];
  }

  const year = walletDetailState.year;
  const timeline = getFinanceYearTimeline(user, year);
  const totals = getFinanceYearTotals(user, year);
  const editingRecord = walletDetailState.editingRecordId ? getWalletEditableRecord(user, walletDetailState.editingRecordId) : null;
  const formType = editingRecord ? editingRecord.recordType : 'payment';
  const formMonthRef = editingRecord ? editingRecord.monthRef : getMonthKeyFromParts(year, 1);
  const formTimestamp = editingRecord ? editingRecord.timestamp : new Date(year, 0, 1, 12, 0, 0, 0).getTime();
  const formStatus = editingRecord ? normalizeFinanceRecordStatus(editingRecord.status, '') : '';

  content.innerHTML = `
    <div class="wallet-detail-header">
      <div>
        <span class="wallet-detail-eyebrow">Historico anual e detalhamento completo</span>
        <h2>${escapeHtml(getUserDisplayName(user))}</h2>
        <p>Visualizacao premium com pagamentos, dividas, datas exatas, distribuicao mensal e historico separado por ano.</p>
      </div>
      <div class="wallet-detail-header-actions">
        <button class="btn-ghost" type="button" data-wallet-close="true">Fechar</button>
      </div>
    </div>

    <div class="wallet-detail-summary">
      <article><span>Total acumulado</span><strong>${formatCurrency(totals.accumulatedTotal)}</strong></article>
      <article><span>Total pago no ano</span><strong>${formatCurrency(totals.paidYear)}</strong></article>
      <article><span>Total de dividas no ano</span><strong>${formatCurrency(totals.debtYear)}</strong></article>
      <article><span>Status geral</span><strong>${escapeHtml(totals.generalStatus)}</strong></article>
    </div>

    <div class="wallet-year-switcher">
      ${availableYears.map((item) => `
        <button class="${item === year ? 'active' : ''}" type="button" data-wallet-year="${item}">${item}</button>
      `).join('')}
    </div>

    ${canManageWallets() ? `
      <section class="wallet-detail-editor">
        <div class="wallet-detail-editor-head">
          <h3>${editingRecord ? 'Editar registro' : 'Adicionar registro'}</h3>
          <p>Admins podem ajustar datas, valores, meses, status, detalhes e historico anual.</p>
        </div>
        <form id="wallet-detail-form" class="wallet-detail-form">
          <input id="wallet-detail-record-id" type="hidden" value="${editingRecord ? editingRecord.id : ''}" />
          <label>
            <span>Tipo</span>
            <select id="wallet-detail-entry-type">
              <option value="payment" ${formType === 'payment' ? 'selected' : ''}>Pagamento</option>
              <option value="debt" ${formType === 'debt' ? 'selected' : ''}>Divida</option>
              <option value="fine" ${formType === 'fine' ? 'selected' : ''}>Multa/Juros</option>
              <option value="loan" ${formType === 'loan' ? 'selected' : ''}>Emprestimo</option>
              <option value="adjustment" ${formType === 'adjustment' ? 'selected' : ''}>Ajuste</option>
            </select>
          </label>
          <label>
            <span>Mes de referencia</span>
            <select id="wallet-detail-entry-month">
              ${getFinanceChartMonthKeys(year).map((monthKey) => `
                <option value="${monthKey}" ${monthKey === formMonthRef ? 'selected' : ''}>${escapeHtml(getMonthLongLabelFromKey(monthKey))}</option>
              `).join('')}
            </select>
          </label>
          <label>
            <span>Valor</span>
            <input id="wallet-detail-entry-amount" type="number" min="0.01" step="0.01" value="${editingRecord ? Number(editingRecord.amount).toFixed(2) : ''}" />
          </label>
          <label>
            <span>Data exata</span>
            <input id="wallet-detail-entry-date" type="datetime-local" value="${formatDateTimeInputValue(formTimestamp)}" />
          </label>
          <label>
            <span>Status</span>
            <select id="wallet-detail-entry-status">
              <option value="" ${formStatus === '' ? 'selected' : ''}>Automatico</option>
              <option value="open" ${formStatus === 'open' ? 'selected' : ''}>Em aberto</option>
              <option value="partial" ${formStatus === 'partial' ? 'selected' : ''}>Parcial</option>
              <option value="paid" ${formStatus === 'paid' ? 'selected' : ''}>Quitado</option>
              <option value="scheduled" ${formStatus === 'scheduled' ? 'selected' : ''}>Sem registro</option>
            </select>
          </label>
          <label class="full">
            <span>Observacao curta</span>
            <input id="wallet-detail-entry-note" type="text" maxlength="180" value="${escapeHtml(editingRecord ? editingRecord.note : '')}" placeholder="Ex: Pagamento confirmado por PIX" />
          </label>
          <label class="full">
            <span>Detalhamento</span>
            <textarea id="wallet-detail-entry-details" maxlength="240" placeholder="Descreva o que foi pago, a divida do periodo, ajustes ou observacoes.">${escapeHtml(editingRecord ? editingRecord.details : '')}</textarea>
          </label>
          <div class="wallet-detail-form-actions">
            <button class="btn-primary" type="submit">${editingRecord ? 'Salvar alteracoes' : 'Adicionar registro'}</button>
            <button class="btn-ghost" id="wallet-detail-form-clear" type="button">Limpar</button>
          </div>
        </form>
      </section>
    ` : ''}

    <section class="wallet-timeline-grid">
      ${timeline.map((month) => `
        <article class="wallet-month-card">
          <div class="wallet-month-head">
            <div>
              <strong>${escapeHtml(month.label)}</strong>
              <small>${month.totalRecords > 0 ? `${month.totalRecords} registro(s)` : 'Sem movimentacao'}</small>
            </div>
            <span class="wallet-status-badge ${escapeHtml(month.status)}">${escapeHtml(getWalletStatusLabel(month.status))}</span>
          </div>
          <div class="wallet-month-stats">
            <span>Pago: <strong>${formatCurrency(month.paidTotal)}</strong></span>
            <span>Dividas: <strong>${formatCurrency(month.debtTotal)}</strong></span>
          </div>
          <div class="wallet-month-columns">
            <div>
              <h4>Pagamentos</h4>
              ${renderWalletRecordList(month.payments, 'payment')}
            </div>
            <div>
              <h4>Dividas</h4>
              ${renderWalletRecordList(month.debts, 'debt')}
            </div>
          </div>
        </article>
      `).join('')}
  ...
  content.querySelectorAll('[data-wallet-year]').forEach((button) => {
    button.addEventListener('click', () => {
      walletDetailState.year = Number(button.dataset.walletYear) || year;
      walletDetailState.editingRecordId = null;
      renderWalletDetailsModal();
    });
  });

  content.querySelectorAll('[data-wallet-record-edit]').forEach((button) => {
    button.addEventListener('click', () => {
      walletDetailState.editingRecordId = Number(button.dataset.walletRecordEdit);
      renderWalletDetailsModal();
    });
  });

  content.querySelectorAll('[data-wallet-record-delete]').forEach((button) => {
    button.addEventListener('click', () => {
      const recordId = Number(button.dataset.walletRecordDelete);
      const targetUser = getUserByUsername(walletDetailState.username);
      if (!targetUser) return;
      targetUser.financeHistory = normalizeFinanceHistoryCollection(targetUser.financeHistory).filter((entry) => entry.id !== recordId);
      recalculateUserFinanceSnapshot(targetUser);
      targetUser.financeUpdatedAt = Date.now();
      walletDetailState.editingRecordId = null;
      saveData({ immediate: true });
      addLog(`Excluiu registro financeiro #${recordId} de ${targetUser.username}`);
      rerenderCurrentPagePreservingWalletModal();
    });
  });

  const clearButton = document.getElementById('wallet-detail-form-clear');
  if (clearButton) {
    clearButton.addEventListener('click', () => {
      walletDetailState.editingRecordId = null;
      renderWalletDetailsModal();
  ...
  const form = document.getElementById('wallet-detail-form');
  if (form) {
    form.addEventListener('submit', (event) => {
      event.preventDefault();
      const targetUser = getUserByUsername(walletDetailState.username);
      if (!targetUser) return;
      const recordId = Number(document.getElementById('wallet-detail-record-id').value || 0);
      const recordType = normalizeFinanceRecordType(document.getElementById('wallet-detail-entry-type').value, 'payment');
      const amount = toPositiveAmount(document.getElementById('wallet-detail-entry-amount').value);
      const monthRef = document.getElementById('wallet-detail-entry-month').value;
      const timestamp = parseDateTimeInputValue(document.getElementById('wallet-detail-entry-date').value);
      const status = normalizeFinanceRecordStatus(document.getElementById('wallet-detail-entry-status').value, '');
      const note = document.getElementById('wallet-detail-entry-note').value.trim();
      const details = document.getElementById('wallet-detail-entry-details').value.trim();
      if (!amount || !parseMonthKey(monthRef) || !timestamp) {
        showNotification('Preencha valor, mes e data exata do registro.', 'warning');
        return;
      }

      const eventType = recordType === 'payment'
        ? 'payment'
        : (recordType === 'loan' ? 'loan' : (recordType === 'adjustment' ? 'adjustment' : 'charge'));
      const existing = recordId ? getWalletEditableRecord(targetUser, recordId) : null;
      const nextEntry = normalizeFinanceHistoryEntry({
      ...
      const history = normalizeFinanceHistoryCollection(targetUser.financeHistory);
      targetUser.financeHistory = existing
        ? history.map((entry) => (entry.id === existing.id ? nextEntry : entry))
        : [...history, nextEntry];
      recalculateUserFinanceSnapshot(targetUser);
      targetUser.financeUpdatedAt = Date.now();
      walletDetailState.editingRecordId = null;
      saveData({ immediate: true });
      addLog(`${existing ? 'Editou' : 'Adicionou'} registro financeiro em ${targetUser.username} (${getFinanceRecordTypeLabel(recordType).toLowerCase()})`);
      rerenderCurrentPagePreservingWalletModal();
    });
  }
}

function renderWalletSpotlightCard(member, options = {}) {
  const safeYear = Number(options.year) || FINANCE_CHART_YEAR;
  const yearTotals = getFinanceYearTotals(member, safeYear);
  const monthSummary = getUserFinanceMonthSummary(member, getCurrentMonthKey());
  const canvasId = options.canvasId || `wallet-card-${member.username}`;
  const showAdminActions = options.showAdminActions === true;
  return `
    <article class="wallet-spotlight-card ${showAdminActions ? 'admin' : 'viewer'}">
      <div class="wallet-spotlight-head">
        <div class="wallet-spotlight-identity">
          ${renderUserAvatar(member, { size: 'xs' })}
          <div>
            <strong>${escapeHtml(getUserDisplayName(member))}</strong>
            <small>@${escapeHtml(member.username)} - ${escapeHtml(roleLabel(member.role))}</small>
        ...
        <span class="wallet-status-badge ${escapeHtml(yearTotals.openDebt <= 0 ? 'paid' : 'open')}">${escapeHtml(yearTotals.openDebt <= 0 ? 'Em dia' : 'Com saldo em aberto')}</span>
      </div>
      <canvas id="${canvasId}" class="member-wallet-canvas wallet-spotlight-canvas" data-wallet-open="${escapeHtml(member.username)}" data-wallet-year="${safeYear}"></canvas>
      <div class="wallet-spotlight-stats">
        <span>Acumulado <strong>${formatCurrency(yearTotals.accumulatedTotal)}</strong></span>
        <span>Pago no ano <strong>${formatCurrency(yearTotals.paidYear)}</strong></span>
        <span>Dividas do ano <strong>${formatCurrency(yearTotals.debtYear)}</strong></span>
        <span>Pago no mes <strong>${formatCurrency(monthSummary.paid)}</strong></span>
      </div>
      <div class="wallet-spotlight-actions">
        <button class="btn-secondary" type="button" data-wallet-open="${escapeHtml(member.username)}" data-wallet-year="${safeYear}">Ver detalhes</button>
        ${showAdminActions ? `<button class="btn-ghost" type="button" data-wallet-quick="payment" data-wallet-user="${escapeHtml(member.username)}">Pagamento rapido</button>` : ''}
        ${showAdminActions ? `<button class="btn-ghost" type="button" data-wallet-quick="debt" data-wallet-user="${escapeHtml(member.username)}">Divida rapida</button>` : ''}
      </div>
    </article>
  `;
}

function bindWalletQuickActions(root = document) {
  root.querySelectorAll('[data-wallet-quick]').forEach((button) => {
    button.addEventListener('click', () => {
      const user = getUserByUsername(button.dataset.walletUser);
      if (!user) {
        showNotification('Usuario nao encontrado.', 'warning');
        return;
      }
      const mode = button.dataset.walletQuick;
      const rawAmount = prompt(mode === 'payment' ? `Pagamento para ${user.username}:` : `Divida para ${user.username}:`, mode === 'payment' ? '100.00' : '50.00');
      ...
      const timestampPrompt = promptFinanceTimestamp(`Data/hora do registro para ${user.username}`);
      if (timestampPrompt.cancelled) return;
      if (timestampPrompt.invalid) {
        showNotification('Data/hora invalida.', 'warning');
        return;
      }
      const monthRef = getMonthKeyFromTimestamp(timestampPrompt.timestamp);
      const result = registerFinanceEvent(
        user,
        mode === 'payment' ? 'payment' : 'charge',
        amount,
        mode === 'payment' ? 'Pagamento rapido pelo admin' : 'Divida rapida pelo admin',
        null,
        {
          timestamp: timestampPrompt.timestamp,
          monthRef,
          recordType: mode === 'payment' ? 'payment' : 'debt',
          status: mode === 'payment' ? 'paid' : ''
        }
      );
      if (!result.ok) {
        showNotification(result.message || 'Nao foi possivel registrar.', 'warning');
        return;
      }
      addLog(`${mode === 'payment' ? 'Registrou pagamento rapido' : 'Registrou divida rapida'} para ${user.username} (${formatCurrency(result.amount)})`);
      rerenderCurrentPagePreservingWalletModal();
    });
  });
}
...
function getNavigationLinks() {
  ...
  if (session && canUseStealthChat()) {
    links.push({ id: 'stealthChat', label: 'Chat Dispiagem' });
  }
  if (session && canAccessStaffChat()) {
    links.push({ id: 'staffChat', label: 'Chat interno' });
  }
...
function renderActivityCenter() {
  ...
  runWalletRedesignMigration();
  const year = FINANCE_CHART_YEAR;
  const todayMonth = getCurrentMonthKey();
  const walletUsers = getWalletParticipants(users);
  const yearPaid = walletUsers.reduce((sum, item) => sum + getFinanceYearTotals(item, year).paidYear, 0);
  const yearDebt = walletUsers.reduce((sum, item) => sum + getFinanceYearTotals(item, year).debtYear, 0);
  const tasksAtRisk = getUserTasks()
    .filter((task) => task.status !== 'done')
    .slice()
    .sort((a, b) => String(a.dueDate || '9999-12-31').localeCompare(String(b.dueDate || '9999-12-31')))
  ...
  const recentAnnouncements = announcements
    ...
    .sort((a, b) => b.createdAt - a.createdAt)
    .slice(0, 4);
  const recentWalletEvents = getRecentWalletEvents(8, walletUsers);
  const onlineUsers = getOnlinePresenceList().slice(0, 8);

  const tasksHtml = tasksAtRisk.length
    ? tasksAtRisk.map((task) => `
        <article class="result-card">
          <div class="result-card-head">
            <strong>${escapeHtml(task.title)}</strong>
          ...
          <p>${escapeHtml(toShortText(task.description || 'Sem descricao adicional.', 130))}</p>
          <small>${task.dueDate ? `Entrega em ${escapeHtml(task.dueDate)}` : 'Sem prazo definido'}</small>
        </article>
      `).join('')
    : '<div class="empty-state">Nenhuma tarefa critica agora.</div>';

  const announcementsHtml = recentAnnouncements.length
    ? recentAnnouncements.map((item) => `
        <article class="result-card">
          <div class="result-card-head">
            <strong>${escapeHtml(item.title)}</strong>
  ...
  const walletEventsHtml = recentWalletEvents.length
    ? recentWalletEvents.map((item) => `
        <article class="wallet-feed-item">
          <div>
            <strong>${escapeHtml(getUserDisplayName(item.user))}</strong>
            <p>${escapeHtml(getFinanceRecordTypeLabel(item.entry.recordType))} ${formatCurrency(item.entry.amount)} | ${escapeHtml(getMonthLongLabelFromKey(item.monthRef))}</p>
          </div>
          <small>${formatDateTime(item.entry.timestamp)}</small>
        </article>
      `).join('')
    : '<div class="empty-state">Sem movimentacoes financeiras recentes.</div>';
  ...
  els.content.innerHTML = `
    <section class="wallet-overview-hero">
      <div class="wallet-overview-copy">
        <span class="hero-kicker">Central limpa e orientada ao fluxo real</span>
        <h2>Operacao, carteira e comunicacao em uma leitura so.</h2>
        <p>Tickets e solicitacoes saem do foco. Aqui ficam o que realmente ajuda a decidir rapido: pagamentos, agenda, comunicados e presenca do time.</p>
        <div class="hero-actions">
          <button class="btn-primary" type="button" data-go="${canManageWallets() ? 'walletControl' : 'dashboard'}">Abrir carteiras</button>
          <button class="btn-secondary" type="button" data-go="tasks">Ver tarefas</button>
          <button class="btn-ghost" type="button" data-go="${session.role === 'superadmin' || session.role === 'inteligencia' ? 'intelCenter' : 'announcements'}">Abrir central intel</button>
        </div>
      </div>
      <div class="wallet-overview-stats">
        <article><span>Pago em ${year}</span><strong>${formatCurrency(yearPaid)}</strong></article>
        <article><span>Dividas em ${year}</span><strong>${formatCurrency(yearDebt)}</strong></article>
        <article><span>Pago no mes atual</span><strong>${formatCurrency(getAggregateMonthFinanceSummary(todayMonth, walletUsers).paid)}</strong></article>
        <article><span>Pessoas online</span><strong>${onlineUsers.length}</strong></article>
      </div>
    </section>

    <section class="panel wallet-premium-panel">
      <div class="panel-header">
        <div>
          <h2 class="panel-title">Movimento financeiro recente</h2>
          <p class="panel-subtitle">Entradas, pagamentos e historico anual organizados por pessoa.</p>
        </div>
      </div>
      <div class="wallet-feed-list">
        ${walletEventsHtml}
      </div>
    </section>
    ...
    <section class="split-grid">
      <section class="panel wallet-premium-panel">
        <div class="panel-header">
          <div>
            <h3 class="panel-title">Tarefas e prioridades</h3>
            <p class="panel-subtitle">O que merece atencao hoje sem poluicao visual.</p>
          </div>
        </div>
        <div class="result-grid">
          ${tasksHtml}
      ...
      <section class="panel wallet-premium-panel">
        <div class="panel-header">
          <div>
            <h3 class="panel-title">Comunicados recentes</h3>
            <p class="panel-subtitle">Avisos relevantes para a operacao e para o financeiro.</p>
          </div>
        </div>
        <div class="result-grid">
          ${announcementsHtml}
    ...
    <section class="panel wallet-premium-panel">
      <div class="panel-header">
        <div>
          <h3 class="panel-title">Presenca do time</h3>
          <p class="panel-subtitle">Quem esta online e em qual tela agora.</p>
        </div>
      </div>
      <div class="table-wrap">
        <table>
          <thead>
            <tr><th>Usuario</th><th>Papel</th><th>Tela</th><th>Ativo ha</th></tr>
          </thead>
          <tbody>${onlineHtml}</tbody>
        </table>
      </div>
    </section>
  `;
...
function renderDashboard() {
  ...
  const financeChartType = getFinanceChartType();
  const walletUsers = getWalletParticipants(users);
  const aggregateSeries = buildAggregateFinanceSeries(12, getMonthKeyFromParts(year, 12), walletUsers);
  const aggregateMonth = getAggregateMonthFinanceSummary(currentMonthKey, walletUsers);
  const aggregateYearPaid = walletUsers.reduce((sum, item) => sum + getFinanceYearTotals(item, year).paidYear, 0);
  const aggregateYearDebt = walletUsers.reduce((sum, item) => sum + getFinanceYearTotals(item, year).debtYear, 0);
  const myYearTotals = getFinanceYearTotals(user, year);
  const myMonthSummary = getUserFinanceMonthSummary(user, currentMonthKey);
  const recentWalletEvents = getRecentWalletEvents(6, walletUsers);
  const dueSoon = getUserTasks()
    .filter((task) => task.status !== 'done')
    .slice()
    .sort((a, b) => String(a.dueDate || '9999-12-31').localeCompare(String(b.dueDate || '9999-12-31')))
    .slice(0, 4);
  const recentAnnouncements = announcements
    ...
    .sort((a, b) => b.createdAt - a.createdAt)
    .slice(0, 3);
  const canViewSharedWalletCharts = canManageWallets() || (session.role === 'member' && isWalletChartsPublicForMembers());
  const sharedMembers = canViewSharedWalletCharts
    ? walletUsers.filter((member) => canUserHaveWalletChart(member)).slice().sort((a, b) => (b.totalPaid || 0) - (a.totalPaid || 0))
    : [];
  const openCentralTarget = session.role === 'superadmin' || session.role === 'inteligencia' ? 'intelCenter' : 'activityCenter';

  const dueSoonHtml = dueSoon.length
    ? dueSoon.map((task) => `<div class="kpi-item"><strong>${escapeHtml(task.title)}</strong><span>${task.dueDate ? `Entrega em ${escapeHtml(task.dueDate)}` : 'Sem prazo definido'}</span></div>`).join('')
    : '<div class="empty-state">Sem tarefas prioritarias agora.</div>';

  const announcementsHtml = recentAnnouncements.length
    ? recentAnnouncements.map((item) => `<div class="announcement"><div class="announcement-header"><strong>${escapeHtml(item.title)}</strong><small>${formatDateTime(item.createdAt)}</small></div><p>${escapeHtml(toShortText(item.content, 150))}</p></div>`).join('')
    : '<div class="empty-state">Sem comunicados recentes.</div>';

  const walletFeedHtml = recentWalletEvents.length
    ? recentWalletEvents.map((item) => `
        <article class="wallet-feed-item">
          <div>
            <strong>${escapeHtml(getUserDisplayName(item.user))}</strong>
  ...
  const personalChartHtml = canUserHaveWalletChart(user)
    ? '<canvas id="wallet-canvas" class="wallet-canvas wallet-canvas-clickable" data-wallet-open="' + escapeHtml(user.username) + '" data-wallet-year="' + year + '"></canvas>'
    : '<div class="empty-state">Seu perfil nao possui grafico individual ativo. O historico completo segue acessivel nas carteiras autorizadas.</div>';

  els.content.innerHTML = `
    <section class="wallet-overview-hero">
      <div class="wallet-overview-copy">
        <span class="hero-kicker">Ano ativo ${year} | ${escapeHtml(getFinanceChartScopeLabel())}</span>
        <h2>Painel financeiro com leitura limpa, detalhada e profissional.</h2>
        <p>O foco agora fica na carteira, no historico anual, na clareza dos blocos e na navegacao simples tanto no celular quanto no PC.</p>
        <div class="hero-actions">
          <button id="dashboard-open-central" class="btn-primary" type="button">Abrir central</button>
          <button id="dashboard-refresh" class="btn-secondary" type="button">Atualizar painel</button>
        </div>
      </div>
      <div class="wallet-overview-stats">
        <article><span>Pago no ano</span><strong>${formatCurrency(aggregateYearPaid)}</strong></article>
        <article><span>Dividas no ano</span><strong>${formatCurrency(aggregateYearDebt)}</strong></article>
        <article><span>Pago no mes</span><strong>${formatCurrency(aggregateMonth.paid)}</strong></article>
        <article><span>Saldo em aberto</span><strong>${formatCurrency(aggregateMonth.debt)}</strong></article>
      </div>
    </section>
    ...
    <section class="panel wallet-premium-panel">
      <div class="panel-header">
        <div>
          <h2 class="panel-title">Carteira consolidada da operacao</h2>
          <p class="panel-subtitle">Grafico anual sempre fechado de janeiro a dezembro, com historico automatico por ano.</p>
        </div>
        <label class="chart-toolbar-field" for="dashboard-chart-type">
          <span>Grafico</span>
          <select id="dashboard-chart-type">
      ...
      <div class="chart-period-note">Ano ativo: <strong>${year}</strong> | Meses exibidos: <strong>Janeiro a Dezembro</strong></div>
      <canvas id="tp-total-canvas" class="wallet-canvas tp-total-canvas"></canvas>
      <div class="wallet-summary-grid">
        <div class="kpi-item"><strong>Total pago no ano</strong><span>${formatCurrency(aggregateYearPaid)}</span></div>
        <div class="kpi-item"><strong>Total de dividas no ano</strong><span>${formatCurrency(aggregateYearDebt)}</span></div>
        <div class="kpi-item"><strong>Pago no mes</strong><span>${formatCurrency(aggregateMonth.paid)}</span></div>
        <div class="kpi-item"><strong>Carteira acumulada</strong><span>${formatCurrency(walletUsers.reduce((sum, item) => sum + (Number(item.totalPaid) || 0), 0))}</span></div>
      </div>
    </section>

    <section class="split-grid wallet-main-grid">
      <section class="panel wallet-premium-panel">
        <div class="panel-header">
          <div>
            <h3 class="panel-title">Minha carteira</h3>
            <p class="panel-subtitle">Resumo claro, historico do ano e detalhamento por clique.</p>
          </div>
        </div>
        ${personalChartHtml}
        <div class="wallet-summary-grid">
          <div class="kpi-item"><strong>Total acumulado</strong><span>${formatCurrency(myYearTotals.accumulatedTotal)}</span></div>
          <div class="kpi-item"><strong>Pago no ano</strong><span>${formatCurrency(myYearTotals.paidYear)}</span></div>
          <div class="kpi-item"><strong>Dividas no ano</strong><span>${formatCurrency(myYearTotals.debtYear)}</span></div>
          <div class="kpi-item"><strong>Pago no mes</strong><span>${formatCurrency(myMonthSummary.paid)}</span></div>
        </div>
        <form id="dashboard-payment-form" class="wallet-inline-form">
          <label>
            <span>Registrar pagamento</span>
            <input id="dashboard-payment-amount" type="number" min="0.01" step="0.01" placeholder="Ex: 100.00" />
          </label>
          <label>
            <span>Observacao</span>
            <input id="dashboard-payment-note" type="text" maxlength="120" placeholder="PIX, dinheiro, transferencia..." />
          </label>
          <div class="wallet-inline-form-actions">
            <button class="btn-primary" type="submit">Salvar pagamento</button>
            <button class="btn-ghost" type="button" data-wallet-open="${escapeHtml(user.username)}" data-wallet-year="${year}">Ver historico completo</button>
          </div>
        </form>
      </section>
      ...
      <section class="panel wallet-premium-panel">
        <div class="panel-header">
          <div>
            <h3 class="panel-title">Movimento recente</h3>
            <p class="panel-subtitle">Ultimos registros da operacao financeira.</p>
          </div>
        </div>
        <div class="wallet-feed-list">
          ${walletFeedHtml}
    ...
    ${sharedMembers.length ? `
      <section class="panel wallet-premium-panel">
        <div class="panel-header">
          <div>
            <h3 class="panel-title">Carteiras por pessoa</h3>
            <p class="panel-subtitle">Clique em qualquer grafico para abrir o detalhamento completo com animacao e historico organizado.</p>
          </div>
        </div>
        <div class="wallet-spotlight-grid">
          ${sharedMembers.map((member, index) => renderWalletSpotlightCard(member, { year, canvasId: `dashboard-member-chart-${index}` })).join('')}
        </div>
      </section>
    ` : ''}
    ...
    <section class="split-grid">
      ...
      <section class="panel wallet-premium-panel">
        <h3 class="panel-title">Comunicados recentes</h3>
        <div class="timeline">${announcementsHtml}</div>
      </section>
    </section>
  `;

  drawFinanceSeriesCanvasAnimated(document.getElementById('tp-total-canvas'), aggregateSeries, {
    secondaryKey: 'charge',
  ...
  if (canUserHaveWalletChart(user)) {
    drawWalletCanvas(document.getElementById('wallet-canvas'), user, {
      anchorMonthKey: getMonthKeyFromParts(year, 12),
      animate: true,
      paidLabel: 'Pago',
      debtLabel: 'Dividas'
    });
  ...
  sharedMembers.forEach((member, index) => {
    const canvas = document.getElementById(`dashboard-member-chart-${index}`);
    drawWalletCanvas(canvas, member, {
      anchorMonthKey: getMonthKeyFromParts(year, 12),
      animate: true,
      paidLabel: 'Pago',
      debtLabel: 'Dividas'
  ...
  document.getElementById('dashboard-open-central').addEventListener('click', () => navigate(openCentralTarget));
  document.getElementById('dashboard-refresh').addEventListener('click', () => renderDashboard());
  document.getElementById('dashboard-chart-type').addEventListener('change', (event) => {
    setFinanceChartType(event.target.value);
  });
  document.getElementById('dashboard-payment-form').addEventListener('submit', (event) => {
    event.preventDefault();
    const amount = toPositiveAmount(document.getElementById('dashboard-payment-amount').value);
    const note = document.getElementById('dashboard-payment-note').value.trim();
    if (!amount) {
      showNotification('Informe um valor de pagamento valido.', 'warning');
      return;
    }
    const result = registerFinanceEvent(user, 'payment', amount, note || 'Pagamento pelo painel', null, {
      monthRef: getCurrentMonthKey(),
      recordType: 'payment',
      status: 'paid'
    });
    if (!result.ok) {
      showNotification(result.message || 'Falha ao salvar o pagamento.', 'warning');
      return;
    }
    addLog(`Registrou pagamento proprio de ${formatCurrency(result.amount)} no painel`);
    renderDashboard();
  });

  bindWalletDetailTriggers(els.content);
}
...
function renderWalletControl() {
  ...
  const chartTheme = getActiveChartTheme();
  const financeChartType = getFinanceChartType();
  const chartsArePublic = isWalletChartsPublicForMembers();
  const members = getWalletParticipants(users).slice().sort((a, b) => a.username.localeCompare(b.username));
  const aggregateSeries = buildAggregateFinanceSeries(12, getMonthKeyFromParts(year, 12), members);
  const aggregateMonth = getAggregateMonthFinanceSummary(getCurrentMonthKey(), members);
  const aggregateYearPaid = members.reduce((sum, item) => sum + getFinanceYearTotals(item, year).paidYear, 0);
  const aggregateYearDebt = members.reduce((sum, item) => sum + getFinanceYearTotals(item, year).debtYear, 0);
  const recentWalletEvents = getRecentWalletEvents(10, members);
  const lateCandidates = buildLateChargeCandidates(members, Date.now()).filter((entry) => entry.eligible);

  els.content.innerHTML = `
    <section class="wallet-overview-hero">
      <div class="wallet-overview-copy">
        <span class="hero-kicker">Painel administrativo da carteira</span>
        <h2>Edicao completa com visual limpo, historico anual e detalhes por pessoa.</h2>
        <p>Os admins podem adicionar, editar, excluir e corrigir pagamentos, dividas, meses, datas e observacoes sem desmontar a estrutura atual do sistema.</p>
        <div class="hero-actions">
          <button id="wallet-admin-refresh" class="btn-primary" type="button">Atualizar</button>
          <button id="wallet-admin-visibility" class="btn-secondary" type="button" ${isEsther() ? '' : 'disabled'}>${chartsArePublic ? 'Privar graficos' : 'Publicar graficos'}</button>
        </div>
      </div>
      <div class="wallet-overview-stats">
        <article><span>Pago em ${year}</span><strong>${formatCurrency(aggregateYearPaid)}</strong></article>
        <article><span>Dividas em ${year}</span><strong>${formatCurrency(aggregateYearDebt)}</strong></article>
        <article><span>Pago no mes</span><strong>${formatCurrency(aggregateMonth.paid)}</strong></article>
        <article><span>Atrasos elegiveis</span><strong>${lateCandidates.length}</strong></article>
      </div>
    </section>
    ...
    <section class="panel wallet-premium-panel">
      <div class="panel-header">
        <div>
          <h2 class="panel-title">Visao consolidada</h2>
          <p class="panel-subtitle">Ano atual sempre exibido com janeiro a dezembro, sem truncar meses futuros.</p>
        </div>
        <label class="chart-toolbar-field" for="wallet-control-chart-type">
          <span>Grafico</span>
          <select id="wallet-control-chart-type">
      ...
      <div class="chart-period-note">Ano ativo: <strong>${year}</strong> | Visibilidade dos graficos: <strong>${chartsArePublic ? 'Publica para membros' : 'Privada'}</strong></div>
      <canvas id="wallet-control-total-canvas" class="wallet-canvas tp-total-canvas"></canvas>
      <div class="wallet-summary-grid">
        <div class="kpi-item"><strong>Total pago no ano</strong><span>${formatCurrency(aggregateYearPaid)}</span></div>
        <div class="kpi-item"><strong>Total de dividas no ano</strong><span>${formatCurrency(aggregateYearDebt)}</span></div>
        <div class="kpi-item"><strong>Pago no mes</strong><span>${formatCurrency(aggregateMonth.paid)}</span></div>
        <div class="kpi-item"><strong>Saldo em aberto</strong><span>${formatCurrency(aggregateMonth.debt)}</span></div>
      </div>
    </section>
    ...
    <section class="panel wallet-premium-panel">
      <div class="panel-header">
        <div>
          <h3 class="panel-title">Adicionar registro rapido</h3>
          <p class="panel-subtitle">Use este formulario para criar registros novos sem abrir outra tela.</p>
        </div>
      </div>
      <form id="wallet-admin-quick-form" class="wallet-detail-form">
        <label>
          <span>Pessoa</span>
          <select id="wallet-admin-target">
            ${members.map((member) => `<option value="${escapeHtml(member.username)}">${escapeHtml(getUserDisplayName(member))} (@${escapeHtml(member.username)})</option>`).join('')}
          </select>
        </label>
        <label>
          <span>Tipo</span>
          <select id="wallet-admin-type">
            <option value="payment">Pagamento</option>
            <option value="debt">Divida</option>
            <option value="fine">Multa/Juros</option>
        ...
        <label>
          <span>Mes</span>
          <select id="wallet-admin-month">
            ${getFinanceChartMonthKeys(year).map((monthKey) => `<option value="${monthKey}">${escapeHtml(getMonthLongLabelFromKey(monthKey))}</option>`).join('')}
          </select>
        </label>
        <label>
          <span>Valor</span>
          <input id="wallet-admin-amount" type="number" min="0.01" step="0.01" placeholder="Ex: 100.00" />
        </label>
        <label>
          <span>Data exata</span>
          <input id="wallet-admin-date" type="datetime-local" value="${formatDateTimeInputValue(Date.now())}" />
        </label>
        <label>
          <span>Status</span>
          <select id="wallet-admin-status">
            <option value="">Automatico</option>
            <option value="open">Em aberto</option>
            <option value="partial">Parcial</option>
            <option value="paid">Quitado</option>
            <option value="scheduled">Sem registro</option>
          </select>
        </label>
        <label class="full">
          <span>Observacao</span>
          <input id="wallet-admin-note" type="text" maxlength="180" placeholder="Ex: Registro corrigido pelo admin" />
        </label>
        <label class="full">
          <span>Detalhamento</span>
          <textarea id="wallet-admin-details" maxlength="240" placeholder="Explique o que foi pago ou qual divida foi registrada."></textarea>
        </label>
        <div class="wallet-detail-form-actions">
    ...
    <section class="panel wallet-premium-panel">
      <div class="panel-header">
        <div>
          <h3 class="panel-title">Carteiras por pessoa</h3>
          <p class="panel-subtitle">Clique em qualquer grafico para abrir o historico completo e editar tudo em um unico lugar.</p>
        </div>
      </div>
      <div class="wallet-spotlight-grid">
        ${members.filter((member) => canUserHaveWalletChart(member)).map((member, index) => renderWalletSpotlightCard(member, { year, canvasId: `wallet-control-member-chart-${index}`, showAdminActions: true })).join('')}
      </div>
    </section>
    ...
    <section class="split-grid">
      <section class="panel wallet-premium-panel">
        <div class="panel-header">
          <div>
            <h3 class="panel-title">Atrasos e automacao</h3>
            <p class="panel-subtitle">A funcionalidade continua ativa, mas em uma leitura mais limpa.</p>
          </div>
        </div>
        <div class="wallet-summary-grid">
          <div class="kpi-item"><strong>Elegiveis agora</strong><span>${lateCandidates.length}</span></div>
          <div class="kpi-item"><strong>Potencial imediato</strong><span>${formatCurrency(lateCandidates.reduce((sum, entry) => sum + (Number(entry.totalAmount) || 0), 0))}</span></div>
        </div>
        <div class="hero-actions">
          <button id="wallet-admin-auto-run" class="btn-secondary" type="button">Executar automacao</button>
          <button id="wallet-admin-auto-force" class="btn-ghost" type="button">Forcar execucao</button>
        </div>
      </section>
      ...
      <section class="panel wallet-premium-panel">
        <div class="panel-header">
          <div>
            <h3 class="panel-title">Movimento recente</h3>
            <p class="panel-subtitle">Ultimas entradas para conferencia rapida.</p>
          </div>
        </div>
        <div class="wallet-feed-list">
          ${recentWalletEvents.map((item) => `
  ...
  drawFinanceSeriesCanvasAnimated(document.getElementById('wallet-control-total-canvas'), aggregateSeries, {
    secondaryKey: 'charge',
    chartType: financeChartType,
    paidLabel: 'Pago',
    debtLabel: 'Dividas',
  ...
  members.filter((member) => canUserHaveWalletChart(member)).forEach((member, index) => {
    drawWalletCanvas(document.getElementById(`wallet-control-member-chart-${index}`), member, {
      anchorMonthKey: getMonthKeyFromParts(year, 12),
      animate: true,
      paidLabel: 'Pago',
      debtLabel: 'Dividas'
  ...
  document.getElementById('wallet-admin-refresh').addEventListener('click', () => renderWalletControl());
  document.getElementById('wallet-control-chart-type').addEventListener('change', (event) => setFinanceChartType(event.target.value));
  document.getElementById('wallet-admin-visibility').addEventListener('click', () => {
    if (!isEsther()) return;
    setWalletChartsVisibilityMode(chartsArePublic ? WALLET_CHART_VISIBILITY_PRIVATE : WALLET_CHART_VISIBILITY_PUBLIC, session.username);
    renderWalletControl();
  });
  document.getElementById('wallet-admin-quick-form').addEventListener('submit', (event) => {
    event.preventDefault();
    const target = getUserByUsername(document.getElementById('wallet-admin-target').value);
    const recordType = normalizeFinanceRecordType(document.getElementById('wallet-admin-type').value, 'payment');
    const amount = toPositiveAmount(document.getElementById('wallet-admin-amount').value);
    const monthRef = document.getElementById('wallet-admin-month').value;
    const timestamp = parseDateTimeInputValue(document.getElementById('wallet-admin-date').value);
    const status = normalizeFinanceRecordStatus(document.getElementById('wallet-admin-status').value, '');
    const note = document.getElementById('wallet-admin-note').value.trim();
    const details = document.getElementById('wallet-admin-details').value.trim();
    if (!target || !amount || !parseMonthKey(monthRef) || !timestamp) {
      showNotification('Preencha pessoa, valor, mes e data do registro.', 'warning');
      return;
    }
    const eventType = recordType === 'payment'
      ? 'payment'
      : (recordType === 'loan' ? 'loan' : (recordType === 'adjustment' ? 'adjustment' : 'charge'));
    target.financeHistory = normalizeFinanceHistoryCollection([
      ...normalizeFinanceHistoryCollection(target.financeHistory),
      {
        id: createId(),
        type: eventType,
        amount,
    ...
    recalculateUserFinanceSnapshot(target);
    target.financeUpdatedAt = Date.now();
    saveData({ immediate: true });
    addLog(`Adicionou registro financeiro para ${target.username} pelo admin (${getFinanceRecordTypeLabel(recordType).toLowerCase()})`);
    renderWalletControl();
  });
  document.getElementById('wallet-admin-auto-run').addEventListener('click', () => {
    const result = runWalletLateAutomation({ actor: session ? session.username : 'sistema', referenceTs: Date.now() });
    showNotification(result.message || 'Automacao executada.', result.ok ? 'success' : 'warning');
    renderWalletControl();
  });
  document.getElementById('wallet-admin-auto-force').addEventListener('click', () => {
    const result = runWalletLateAutomation({ actor: session ? session.username : 'sistema', referenceTs: Date.now(), force: true });
    showNotification(result.message || 'Execucao forcada concluida.', result.ok ? 'success' : 'warning');
    renderWalletControl();
  });

  bindWalletDetailTriggers(els.content);
  bindWalletQuickActions(els.content);
}
...
function renderIntelCenter() {
  ...
  const canAudit = canViewLogs();
  const walletUsers = getWalletParticipants(users);
  const onlineUsers = getOnlinePresenceList().slice(0, 10);
  const recentLogs = canAudit ? logs.slice().sort((a, b) => b.timestamp - a.timestamp).slice(0, 12) : [];
  const blockedUsers = users.filter((user) => user.status === 'blocked');
  const recentWalletEvents = getRecentWalletEvents(8, walletUsers);
  const year = FINANCE_CHART_YEAR;
  const yearPaid = walletUsers.reduce((sum, item) => sum + getFinanceYearTotals(item, year).paidYear, 0);
  const yearDebt = walletUsers.reduce((sum, item) => sum + getFinanceYearTotals(item, year).debtYear, 0);
  const debtRanking = walletUsers.slice().sort((a, b) => (b.debt || 0) - (a.debt || 0)).slice(0, 8);

  els.content.innerHTML = `
    <section class="wallet-overview-hero">
      <div class="wallet-overview-copy">
        <span class="hero-kicker">Central Intel preservada, agora mais clara</span>
        <h2>Radar premium para decisao rapida e leitura financeira limpa.</h2>
        <p>A estrutura central continua existindo, mas com menos poluicao: foco em carteira, presenca, logs importantes e acoes administrativas diretas.</p>
        <div class="hero-actions">
          <button id="intel-refresh" class="btn-primary" type="button">Atualizar</button>
        </div>
      </div>
      <div class="wallet-overview-stats">
        <article><span>Pago em ${year}</span><strong>${formatCurrency(yearPaid)}</strong></article>
        <article><span>Dividas em ${year}</span><strong>${formatCurrency(yearDebt)}</strong></article>
        <article><span>Usuarios online</span><strong>${onlineUsers.length}</strong></article>
        <article><span>Bloqueados</span><strong>${blockedUsers.length}</strong></article>
      </div>
    </section>
    ...
    <section class="split-grid">
      <section class="panel wallet-premium-panel">
        <div class="panel-header">
          <div>
            <h3 class="panel-title">Radar financeiro</h3>
            <p class="panel-subtitle">Quem tem mais saldo em aberto e quanto ja acumulou.</p>
          </div>
        </div>
        <div class="table-wrap">
          <table>
            <thead>
              <tr><th>Usuario</th><th>Divida aberta</th><th>Total acumulado</th><th>Pago no ano</th></tr>
            </thead>
            <tbody>
              ${debtRanking.map((item) => {
                const totals = getFinanceYearTotals(item, year);
                return `<tr><td>${renderUserIdentity(item, { size: 'xs', showRole: false })}</td><td>${formatCurrency(item.debt)}</td><td>${formatCurrency(item.totalPaid)}</td><td>${formatCurrency(totals.paidYear)}</td></tr>`;
              }).join('')}
            </tbody>
          </table>
        </div>
      ...
      <section class="panel wallet-premium-panel">
        <div class="panel-header">
          <div>
            <h3 class="panel-title">Movimento recente</h3>
            <p class="panel-subtitle">Ultimos pagamentos e dividas registradas na carteira.</p>
          </div>
        </div>
        <div class="wallet-feed-list">
          ${recentWalletEvents.map((item) => `
    ...
    <section class="split-grid">
      <section class="panel wallet-premium-panel">
        <div class="panel-header">
          <div>
            <h3 class="panel-title">Presenca ao vivo</h3>
            <p class="panel-subtitle">Quem esta online e em qual tela.</p>
          </div>
        </div>
        <div class="table-wrap">
          <table>
            <thead>
              <tr><th>Usuario</th><th>Papel</th><th>Tela</th><th>Ultimo sinal</th></tr>
            </thead>
            <tbody>
              ...
              `).join('') : '<tr><td colspan="4" class="empty-state">Nenhum usuario online no momento.</td></tr>'}
            </tbody>
          </table>
        </div>
      </section>
      ...
      <section class="panel wallet-premium-panel">
        <div class="panel-header">
          <div>
            <h3 class="panel-title">Acoes Intel</h3>
            <p class="panel-subtitle">Alertas globais e desbloqueio rapido, agora em um bloco mais objetivo.</p>
          </div>
        </div>
        <div class="form-grid">
          <div class="full">
            <label for="intel-alert-title">Alerta relampago</label>
            <input id="intel-alert-title" type="text" maxlength="80" value="${escapeHtml(realtimeDrafts.intelAlertTitle || '')}" placeholder="Ex: Revisao geral em 10 minutos" />
          </div>
          <div class="full">
            <label for="intel-alert-content">Mensagem</label>
            <input id="intel-alert-content" type="text" maxlength="180" value="${escapeHtml(realtimeDrafts.intelAlertContent || '')}" placeholder="Mensagem curta para todos os usuarios." />
          </div>
          <div>
            <label for="intel-unblock-user">Desbloqueio rapido</label>
            <select id="intel-unblock-user">
              ${blockedUsers.length ? blockedUsers.map((item) => `<option value="${escapeHtml(item.username)}">${escapeHtml(getUserDisplayName(item))}</option>`).join('') : '<option value="">Sem usuarios bloqueados</option>'}
            </select>
          </div>
          <div class="wallet-detail-form-actions">
            <button id="intel-send-alert" class="btn-secondary" type="button">Enviar alerta</button>
            <button id="intel-unblock-btn" class="btn-primary" type="button" ${blockedUsers.length ? '' : 'disabled'}>Desbloquear</button>
          </div>
        </div>
      </section>
    </section>
      ...
      <section class="panel wallet-premium-panel">
        <div class="panel-header">
          <div>
            ...
            <p class="panel-subtitle">Auditoria enxuta com os eventos mais recentes.</p>
          </div>
        </div>
        <div class="table-wrap">
          <table>
            <thead>
              <tr><th>Quando</th><th>Usuario</th><th>Acao</th></tr>
            </thead>
            <tbody>
              ${recentLogs.length ? recentLogs.map((entry) => `<tr><td>${formatDateTime(entry.timestamp)}</td><td>${renderUserIdentity(entry.user, { size: 'xs', showRole: false })}</td><td>${escapeHtml(entry.action)}</td></tr>`).join('') : '<tr><td colspan="3" class="empty-state">Sem logs recentes.</td></tr>'}
            </tbody>
          </table>
        </div>
      </section>
  ...
  document.getElementById('intel-refresh').addEventListener('click', () => renderIntelCenter());
  document.getElementById('intel-alert-title').addEventListener('input', (event) => {
    realtimeDrafts.intelAlertTitle = event.target.value;
  });
  document.getElementById('intel-alert-content').addEventListener('input', (event) => {
    realtimeDrafts.intelAlertContent = event.target.value;
  });
  document.getElementById('intel-send-alert').addEventListener('click', () => {
    const title = document.getElementById('intel-alert-title').value.trim();
    const content = document.getElementById('intel-alert-content').value.trim();
    if (title.length < 3 || content.length < 3) {
      showNotification('Preencha titulo e mensagem do alerta.', 'warning');
    ...
    saveData({ immediate: true });
    addLog(`Enviou alerta global pela Central Intel: ${title}`);
    realtimeDrafts.intelAlertTitle = '';
    realtimeDrafts.intelAlertContent = '';
    renderIntelCenter();
  });
  document.getElementById('intel-unblock-btn').addEventListener('click', () => {
    const username = document.getElementById('intel-unblock-user').value;
    const target = getUserByUsername(username);
    if (!target) {
      showNotification('Usuario nao encontrado.', 'warning');
      return;
    ...
    target.statusUpdatedAt = Date.now();
    saveData({ immediate: true });
    addLog(`${session.username} desbloqueou ${target.username} pela Central Intel`);
    renderIntelCenter();
  });
}
