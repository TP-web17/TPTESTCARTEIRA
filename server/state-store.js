...
function normalizeFinanceHistory(rawHistory) {
  ...
  return source
    .map((entry) => ({
      ...
      updatedAt: Math.max(Number(entry.updatedAt) || 0, Number(entry.timestamp) || 0),
      monthRef: /^\d{4}-\d{2}$/.test(safeText(entry.monthRef, '')) ? safeText(entry.monthRef, '') : '',
      recordType: ['payment', 'debt', 'fine', 'loan', 'adjustment'].includes(safeText(entry.recordType, '').toLowerCase())
        ? safeText(entry.recordType, '').toLowerCase()
        : '',
      details: safeText(entry.details, '').slice(0, 240),
      status: ['open', 'partial', 'paid', 'scheduled'].includes(safeText(entry.status, '').toLowerCase())
        ? safeText(entry.status, '').toLowerCase()
        : '',
      seedKey: safeText(entry.seedKey, '').slice(0, 80)
    }))
...
function normalizeServerUser(rawUser, configOverrides = {}) {
  ...
  const financeUpdatedAt = Math.max(
    ...
    ...financeHistory.map((entry) => Math.max(0, Number(entry.timestamp) || 0, Number(entry.updatedAt) || 0))
  );
